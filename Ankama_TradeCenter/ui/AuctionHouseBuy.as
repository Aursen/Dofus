package Ankama_TradeCenter.ui
{
    ...
    import d2hooks.BidHouseBuyResult;
    import d2hooks.ObjectModified;
    import d2hooks.BidHouseSelectItemFromRecipe;
    ...
    public class AuctionHouseBuy extends Stock 
    {
        ...
        private var _allItems:Dictionary = new Dictionary(true);
        ...
        private var _neededFollowedTypes:Dictionary = new Dictionary(true);
        private var _followedTypes:Dictionary = new Dictionary(true);
        private var _neededFollowedObjects:Dictionary = new Dictionary(true);
        private var _followedObjects:Dictionary = new Dictionary(true);
        ...
        private var _itemToBuy:Vector.<ItemWrapper> = new Vector.<ItemWrapper>();
        ...
        override public function main(params:Object=null):void
        {
            ...
            sysApi.addHook(BidObjectListUpdate, this.onBidObjectListUpdate);
            sysApi.addHook(BidHouseBuyResult, this.onBidHouseBuyResult);
            sysApi.addHook(ObjectModified, this.onSelectItemFromInventory);
            sysApi.addHook(BidHouseSelectItemFromRecipe, this.onSelectItemFromRecipe);
            ...
            if (this._sellerDescriptor.types.indexOf(DataEnum.ITEM_TYPE_COLLAR) != -1)
            {
            ...
            {
                if (this._sellerDescriptor.types.indexOf(DataEnum.ITEM_TYPE_DRINK) != -1)
                {
                ...
                {
                    if (this._sellerDescriptor.types.indexOf(DataEnum.ITEM_TYPE_WING) != -1)
                    {
                    ...
                    {
                        if (this._sellerDescriptor.types.indexOf(DataEnum.ITEM_TYPE_SMITHMAGIC_RUNE) != -1)
                        {
                        ...
                        {
                            if (this._sellerDescriptor.types.indexOf(DataEnum.ITEM_TYPE_EMPTY_SOULSTONE) != -1)
                            {
                            ...
                            {
                                if (this._sellerDescriptor.types.indexOf(DataEnum.ITEM_TYPE_DRAGOTURKEY_CERTIFICATE) != -1)
                                {
            ...
            this.itemFilterApi.setMaxLevel(this._sellerDescriptor.maxItemLevel);
            this.itemFilterApi.initFilters(this.currentType, this.playerApi.getPlayedCharacterInfo(), this.setCurrentTypeInfos(), sysApi.getData("openedFilterCat"), this.gd_filters, this.gd_itemList, this._sellerDescriptor.types);
        ...
        public function updateCategory(data:*, componentsRef:*, selected:Boolean, line:uint):void
        {
            switch (this.getCatLineType(data, line))
            {
                case CTR_SEARCH:
                    ...
                    {
                        if ((((!(_searchCriteria)) && (!(inp_search.haveFocus))) || (inp_search.text == INPUT_SEARCH_DEFAULT_TEXT)))
                        {
                ...
                case CTR_FILTER_LEVEL:
                    if (this._sellerDescriptor.maxItemLevel < 200)
                    {
        ...
        public function updateItem(data:*, componentsRef:*, selected:Boolean, line:uint):void
        {
            ...
            switch (this.getItemLineType(data, line))
            {
                ...
                case CTR_ITEM_DETAILS:
                    if (data)
                    {
                        ...
                        if (data.hasOwnProperty("searchInput"))
                        {
                            ...
                            {
                                if (((((!(this._specificSearchCriteria)) && (this.inp_specificSearch.text == "")) && (!(this.inp_specificSearch.haveFocus))) || (this.inp_specificSearch.text == this.INPUT_SPECIFIC_SEARCH_DEFAULT_TEXT)))
                                {
                ...
                case CTR_ITEM_EMPTY:
                    ...
                    if (this._sellerDescriptor.maxItemLevel < data.itemWrapper.level)
                    {
                        componentsRef.lbl_itemEmpty.text = uiApi.getText("ui.bidhouse.maximumLevelWarn", this._sellerDescriptor.maxItemLevel);
                    }
                    else
                    {
        ...
        {
            var typeIdToAdd:uint;
            var currentSuperTypeId:uint;
            var typeIdToRemove:uint;
        ...
        private function reloadFilters():void
        {
            ...
            this.inp_maxLevel.text = ((reloadInfos.maxLevelManual > 0) ? reloadInfos.maxLevelManual : this._sellerDescriptor.maxItemLevel);
        ...
        private function setCurrentTypeInfos():Dictionary
        {
            ...
            for each (typeId in this._sellerDescriptor.types)
            {
                ...
                {
                    if (superTypeId == DataEnum.ITEM_SUPERTYPE_MOUNT_EQUIPMENT)
                    {
                    ...
                    {
                        if (typeId == DataEnum.ITEM_TYPE_CATCHING_NET)
                        {
                        ...
                        {
                            if (((superTypeId == DataEnum.ITEM_SUPERTYPE_PET_GHOST) || (typeId == DataEnum.ITEM_TYPE_RECEIPT)))
                            {
                            ...
                            {
                                if ((((((typeId == DataEnum.ITEM_TYPE_PET_POTION) || (typeId == DataEnum.ITEM_TYPE_PETSMOUNT_POTION)) || (typeId == DataEnum.ITEM_TYPE_MOUNT_POTION)) || (typeId == DataEnum.ITEM_TYPE_PET_EGG)) || (typeId == DataEnum.ITEM_TYPE_BREEDING_ITEM)))
                                {
                                ...
                                {
                                    if (typeId == DataEnum.ITEM_TYPE_CEREMONIAL_ITEMS)
                                    {
                                    ...
                                    {
                                        if (((typeId == DataEnum.ITEM_TYPE_SOULSTONE) || (typeId == DataEnum.ITEM_TYPE_EMPTY_SOULSTONE)))
                                        {
                                            if (!allTypes[DataEnum.ITEM_SUPERTYPE_CONSUMABLE])
                                            {
                                                allTypes[DataEnum.ITEM_SUPERTYPE_CONSUMABLE] = [];
        ...
        private function addFollowedType(typeId:uint, follow:Boolean):void
        {
            if (follow)
            {
                if (!this._neededFollowedTypes[typeId])
                {
                    ...
                    sysApi.sendAction(new ExchangeBidHouseType(typeId, follow));
                };
            }
            else
            {
        ...
        private function addAllSubFilterIds(superTypeId:uint):void
        {
            ...
            {
                if (this._sellerDescriptor.types.indexOf(typeId) != -1)
                {
        ...
        {
            var timer:uint;
            var superTypeIdToAdd:uint;
            var subTypeId:uint;
            var superTypeIdToRemove:uint;
            var indexHDVType:int = this._sellerDescriptor.types.indexOf(typeId);
            if (indexHDVType == -1)
            {
                return;
            };
                ...
                else
                {
                    ...
                    for each (superTypeIdToRemove in this.itemFilterApi.getCurrentTypeInfos()[this.itemFilterApi.getCurrentTypeInfosKeyByValue(itemType.superTypeId)])
                    {
                        if (!this.itemFilterApi.checkSubFilters(superTypeIdToRemove))
                        {
                            ...
                            {
                                if (((!(this.containAssociateTypes(superTypeIdToRemove))) || (!(this.containAssociateSubTypes(superTypeIdToRemove)))))
                                {
        ...
        private function displayItemDetails(selectedItem:Object, itemList:Array):void
        {
            ...
            for each (entry in itemList)
            {
                ...
                {
                    if (((entry.isItemDetails) && (entry.hasOwnProperty("empty"))))
                    {
                        tempCats.push(entry);
                        index++;
                    }
                    ...
                    {
                        if (((entry.isItemDetails) && (entry.searchInput)))
                        {
                            tempCats.push(entry);
                            index++;
                        };
            ...
            {
                if ((((!(this.gd_itemList.selectedIndex == myIndex)) && (!(selectedGid == -1))) && (!(selectedItem == null))))
                {
                    this.gd_itemList.silent = true;
                ...
                {
                    if (((((!(this.gd_itemList.selectedIndex == myIndexDetails)) && (!(selectedUid == -1))) && (!(myIndexDetails == 0))) && (!(selectedItem == null))))
                    {
                        this.gd_itemList.silent = true;
        ...
        private function updateItemList(objectIds:Array):void
        {
            ...
            var tmpSuperTypeId:uint;
            var itemAvailable:Boolean;
            ...
            if (((((((this._selectInventory) || (TradeCenter.SEARCH_MODE)) || (this.itemFilterApi.currentFilteredIds().length > 0)) || (this.itemFilterApi.currentCharacIds().length > 0)) || (this.itemFilterApi.currentWeaponEffectIds().length > 0)) || (this.itemFilterApi.currentConsumableEffectIds().length > 0)))
            {
                ...
                while (i < tmpAllItems.length)
                {
                    if (this._sellerDescriptor.types.indexOf(tmpAllItems[i].itemWrapper.typeId) != -1)
                    {
                        tmpIds.push(tmpAllItems[i]);
                    };
                    i++;
        ...
        private function updateItemListWithSpecificSearchCriteria(specificSearchCriteria:String):void
        {
            ...
            {
                if (!specificSearchCriteria)
                {
                    itemsWithCriteria = this._lastItemDetails;
                };
            };
            var k:uint;
            while (k < itemsWithCriteria.length)
            {
        ...
        private function buyItem():void
        {
            ...
            if (this.gd_itemList.selectedItem != null)
            {
                ...
                if (((this._buyPopupName == null) && (price > 0)))
                {
                    ...
                    params.stack = this._sellerDescriptor.quantities[quantityIndex];
                    params.unityPrice = utilApi.kamasToString(Math.round((price / int(this._sellerDescriptor.quantities[quantityIndex]))), "");
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (option == "Sometimes")
                            {
                                blocking = ((this.lastBuyItem) && ((utilApi.stringToKamas(this.lastBuyItem.price) * RELATION_MINI_FOR_WARNING_POPUP) < utilApi.stringToKamas(params.price)));
                            };
                        };
        ...
        private function getSearchAllTypeId():void
        {
            ...
            for each (id in ids)
            {
                ...
                if (this._sellerDescriptor.types.indexOf(item.typeId) != -1)
                {
        ...
        private function createItemDetails(items:Object):Array
        {
            ...
            var searchInput:Boolean;
            for each (item in items)
            {
                i = 0;
                while (i < item.prices.length)
                {
                    if (item.prices[i] > 0)
                    {
        ...
        {
            var openedItem:OpenedItemDetails;
        ...
        {
            var itemToRefresh:OpenedItemDetails;
            var openedItem:OpenedItemDetails;
            ...
            if (((!(this._neededFollowedObjects[itemGID])) && (this._followedObjects[itemGID])))
            {
                ...
                if (this._itemsToRefresh.indexOf(itemGID) != -1)
                {
                    ...
                    this.followItem(((itemGID < 0) ? dataApi.getItemWrapper(itemGID).typeId : items[0].itemWrapper.typeId), itemGID, true);
                    return (true);
                };
                return (false);
            };
        ...
        private function followItem(itemTypeID:uint, itemGID:uint, follow:Boolean):void
        {
            ...
            sysApi.sendAction(new ExchangeBidHouseSearch(itemTypeID, itemGID, follow));
        }

        private function showArrow():void
        {
            var openedItem:OpenedItemDetails;
            var parentObject:OpenedItemDetails;
        ...
        public function onBidObjectTypeListUpdate(objects:Array, typeId:uint):void
        {
            ...
            if (this._currentTypesRequested.length <= 0)
            {
                this.filterItems();
            };
        }
        ...
        {
            var openedItem:OpenedItemDetails;
            var openedItemDetails:OpenedItemDetails;
            var removedItem:OpenedItemDetails;
            var newIndex:uint;
            var itemAlreadyOpen:OpenedItemDetails;
            var delta:uint;
            var info:Object;
            var l:uint;
            ...
            var k:uint;
            while (k < this._openedItemsDetails.length)
            {
                l = 0;
                while (l < this._openedItemsDetails[k].data.length)
                {
        ...
        override public function onRelease(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                default:
                    ...
                    {
                        if (target.name.indexOf("btn_moveToGlobalItemArrow") != -1)
                        {
                            this.unshowArrow();
                        ...
                        else
                        {
                            if (target.name.indexOf("btn_selectFilter") != -1)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (data.id == "consumableEffect")
                                        {
                                            this.itemFilterApi.addConsumableEffect(data.cId);
                                        }
                                        else
                                        {
                                            if (((data.id == "targetEquipment") || (data.id == "targetWeapon")))
                                            {
                                    ...
                                    if (((this.itemFilterApi.currentFilteredIds().length > 0) || (((((this.itemFilterApi.currentFilteredIds().length <= 0) && (this.itemFilterApi.currentCharacIds().length <= 0)) && (this.itemFilterApi.currentWeaponEffectIds().length <= 0)) && (this.itemFilterApi.currentConsumableEffectIds().length <= 0)) && (this.itemFilterApi.currentSkinTargetIds().length <= 0))))
                                    {
                                    ...
                                    {
                                        if (((((this.itemFilterApi.currentCharacIds().length > 0) || (this.itemFilterApi.currentWeaponEffectIds().length > 0)) || (this.itemFilterApi.currentConsumableEffectIds().length > 0)) || (this.itemFilterApi.currentSkinTargetIds().length > 0)))
                                        {
                                            this.filterItems();
                                        };
                                    };
                            ...
                            {
                                if (target.name.indexOf("btn_itemType") != -1)
                                {
                                    if (this._searchAll)
                                    {
                                        this.clearFollows();
                                ...
                                {
                                    if (((!(target.name.indexOf("btn_gridItemDetails") == -1)) && (uiApi.keyIsDown(Keyboard.SHIFT))))
                                    {
                                        sysApi.dispatchHook(InsertHyperlink, this._componentList[target.name].data.itemWrapper);
                                    };
                                };
                            };
                        };
        ...
        override public function onRollOver(target:Object):void
        {
            ...
            else
            {
                ...
                {
                    if (target.name.indexOf("tx_helpLevel") != -1)
                    {
                        uiApi.showTooltip(uiApi.textTooltipInfo(uiApi.getText("ui.bidhouse.maxLevelInfo", this._currentSubArea.area.name, this._sellerDescriptor.maxItemLevel), null, null, 500), target, false, "standard", 7, 1, 3, null, null, null, "TextInfo");
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    if (target.name.indexOf("btn_gridItemDetails") != -1)
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            if (target.name.indexOf("ctr_empty") != -1)
                                            {
        ...
        private function onConfirmBuyObject():void
        {
            ...
            if (this._selectedItem.currentCase > -1)
            {
                ...
                quantity = this._sellerDescriptor.quantities[this._selectedItem.currentCase];
                price = this._selectedItem.prices;
                sysApi.sendAction(new ExchangeBidHouseBuy(this._selectedItem.itemWrapper.objectUID, quantity, price));
            }
            else
            {
        ...
        override public function onKeyUp(target:Object, keyCode:uint):void
        {
            ...
            {
                if (((this.inp_maxLevel) && (this.inp_maxLevel.haveFocus)))
                {
                    this._levelTimer.reset();
                    this._levelTimer.start();
            ...
            {
                if (((this.inp_specificSearch) && (!(this.inp_specificSearch.text == this.INPUT_SPECIFIC_SEARCH_DEFAULT_TEXT))))
                {
        ...
        protected function onLevelTimerComplete(e:TimerEvent):void
        {
            ...
            {
                if (this._currentInputLevel == this.inp_maxLevel)
                {
        ...
        private function onSpecificSearchTimerComplete(e:TimerEvent):void
        {
            ...
            {
                if (this.inp_specificSearch.text.length == 0)
                {
                    if (this._specificSearchCriteria)
                    {
                        this._specificSearchCriteria = null;
...
class OpenedItemDetails 
{
    ...
    public function OpenedItemDetails(item:Object, data:Array, initialIndex:uint, currentIndex:uint)
    {
        this.item = item;
        this.data = data;
