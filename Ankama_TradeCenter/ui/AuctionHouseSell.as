package Ankama_TradeCenter.ui
{
    ...
    public class AuctionHouseSell extends BasicItemCard 
    {
        ...
        private var _currentSameItemsInAuctionHouse:Dictionary = new Dictionary(true);
        ...
        private var _stackChoices:Array = ["1", "10", "100"];
        ...
        override public function main(params:Object=null):void
        {
            ...
            sysApi.addHook(DisplayPinnedItemTooltip, this.onDisplayPinnedTooltip);
        ...
        public function updateItem(data:*, componentsRef:*, selected:Boolean, line:uint):void
        {
            ...
            if (data)
            {
                ...
                days = int(Math.floor((data.unsoldDelay / (24 * 3600))));
                hours = int(Math.floor(((data.unsoldDelay % (24 * 3600)) / 3600)));
                minutes = int(Math.floor((((data.unsoldDelay % (24 * 3600)) % 3600) / 60)));
        ...
        private function updateTax():void
        {
            ...
            if (((this._mode) || (!(currentPrice == this._oldPrice))))
            {
                ...
                else
                {
                    if (this._mode)
                    {
                        this._tax = Math.max(1, Math.round(((currentPrice * this._sellerDescriptor.taxPercentage) / 100)));
                    }
                    else
                    {
                        if (this._oldPrice < currentPrice)
                        {
                            this._tax = Math.max(1, Math.round((((currentPrice - this._oldPrice) * this._sellerDescriptor.taxPercentage) / 100)));
                        }
                        else
                        {
                            this._tax = Math.max(1, Math.round(((currentPrice * this._sellerDescriptor.taxModificationPercentage) / 100)));
                        };
                    };
                };
                ...
                if (this._mode)
                {
                    this.lbl_taxTimeTitle.text = (((uiApi.getText("ui.bidhouse.bigStoreTax") + uiApi.getText("ui.common.colon")) + this._sellerDescriptor.taxPercentage) + "%");
                }
                else
                {
                    this.lbl_taxTimeTitle.text = (((uiApi.getText("ui.bidhouse.bigStoreModificationTax") + uiApi.getText("ui.common.colon")) + this._sellerDescriptor.taxModificationPercentage) + "%");
                };
        ...
        public function getTax(oldPrice:uint, newPrice:uint):uint
        {
            ...
            else
            {
                if (this._mode)
                {
                    this._tax = Math.max(1, Math.round(((newPrice * this._sellerDescriptor.taxPercentage) / 100)));
                }
                else
                {
                    if (oldPrice < newPrice)
                    {
                        this._tax = Math.max(1, Math.round((((newPrice - oldPrice) * this._sellerDescriptor.taxPercentage) / 100)));
                    }
                    else
                    {
                        this._tax = Math.max(1, Math.round(((newPrice * this._sellerDescriptor.taxModificationPercentage) / 100)));
                    };
                };
            };
            return (this._tax);
        ...
        protected function updateLabelQuantitySoldObject():void
        {
            ...
            else
            {
                ...
                this.lbl_quantityObject.text = uiApi.getText("ui.bidhouse.slotOfSale", ((this.gd_itemList.dataProvider.length + "/") + this._sellerDescriptor.maxItemPerAccount));
        ...
        private function searchSameItemsInAuctionHouse(item:Object, autoSelect:Boolean=false):void
        {
            ...
            {
                sysApi.sendAction(new ExchangeBidHouseSearch(this._currentItemSelected.typeId, this._currentItemSelected.objectGID, false));
            };
            this._currentItemSelected = item;
            sysApi.sendAction(new ExchangeBidHouseSearch(item.typeId, item.objectGID, true));
        }
        ...
        {
            var hoursF1:int = int(Math.floor((firstItem.unsoldDelay / 3600)));
            var minutesF1:int = int(Math.floor((((firstItem.unsoldDelay % (24 * 3600)) % 3600) / 60)));
            var hoursF2:int = int(Math.floor((secondItem.unsoldDelay / 3600)));
            var minutesF2:int = int(Math.floor((((secondItem.unsoldDelay % (24 * 3600)) % 3600) / 60)));
            var firstValue:int = ((hoursF1 < 1) ? minutesF1 : hoursF1);
            var secondValue:int = ((hoursF2 < 1) ? minutesF2 : hoursF2);
            if (firstValue > secondValue)
            {
                return (1);
            };
        ...
        {
            var firstValue:int = ((firstItem.itemWrapper.info1) ? firstItem.itemWrapper.info1 : 1);
            var secondValue:int = ((secondItem.itemWrapper.info1) ? secondItem.itemWrapper.info1 : 1);
            if (firstValue > secondValue)
            {
                return (1);
            };
        ...
        {
            var effectinstance:EffectInstanceDice;
        ...
        public function onSellerObjectListUpdate(vendorObjects:Object):void
        {
            ...
            var newItemsToRemove:Array = [];
            while (i < itemsToRemoveCount)
            {
        ...
        public function onSelectItemFromStockItemList(selectedItem:Object):void
        {
            ...
            var days:int = int(Math.floor((selectedItem.unsoldDelay / (24 * 3600))));
            var hours:int = int(Math.floor(((selectedItem.unsoldDelay % (24 * 3600)) / 3600)));
            var minutes:int = int(Math.floor((((selectedItem.unsoldDelay % (24 * 3600)) % 3600) / 60)));
        ...
        private function checkItemToModify():Boolean
        {
            ...
            var firstItemStack:int = ((this._allItemsToRemove[0].itemWrapper.info1) ? this._allItemsToRemove[0].itemWrapper.info1 : 1);
            var i:int = 1;
            while (i < this._allItemsToRemove.length)
            {
                itemStack = ((this._allItemsToRemove[i].itemWrapper.info1) ? this._allItemsToRemove[i].itemWrapper.info1 : 1);
        ...
        public function onBidObjectListUpdate(items:Object, itemGID:int, update:Boolean=false, newObjectType:Boolean=false):void
        {
            ...
            var i:uint;
            var stack:uint;
            var itemInfos:Object;
            var itemType:ItemType;
            var dataProvider:Array;
            var key:Object;
            var displayOtherItems:Boolean;
        ...
        override public function onRollOver(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                default:
                    ...
                    {
                        if (((!(target.name.indexOf("tx_otherItemBg") == -1)) || (!(target.name.indexOf("tx_otherItemIcon") == -1))))
                        {
                            this.displayItemTooltip(this.gd_otherItemList, this._componentList[target.name]);
                        }
                        else
                        {
                            if (((!(target.name.indexOf("lbl_itemName") == -1)) || (!(target.name.indexOf("chk_remove") == -1))))
                            {
                                if (this.currentRollOver)
                                {
                                    this.currentRollOver.state = ((this.currentRollOver.selected) ? StatesEnum.STATE_SELECTED_CLICKED : StatesEnum.STATE_OVER);
                            ...
                            else
                            {
                                ...
                                {
                                    if (target.name.indexOf("lbl_storeTime") != -1)
                                    {
                                        if (this.currentRollOver)
                                        {
                                            this.currentRollOver.state = ((this.currentRollOver.selected) ? StatesEnum.STATE_SELECTED_CLICKED : StatesEnum.STATE_OVER);
        ...
        public function onConfirmModifyMultipleItems():void
        {
            ...
            for each (item in this._allItemsToRemove)
            {
                ...
                {
                    sysApi.sendAction(new ExchangeObjectModifyPriced(item.itemWrapper.objectUID, TradeCenter.QUANTITIES[this._exchangeQuantity], this._price));
        ...
        {
            this._sellPopupName = null;
            sysApi.sendAction(new ExchangeShopStockMouvmentRemove(this._currentObjectToRemove.itemWrapper.objectUID, -(TradeCenter.QUANTITIES[this._exchangeQuantity])));
        ...
        private function onConfirmWithdrawAllObject():void
        {
            ...
            for each (item in this._allItemsToRemove)
            {
                ...
                sysApi.sendAction(new ExchangeShopStockMouvmentRemove(_currentObject.objectUID, -(TradeCenter.QUANTITIES[this._exchangeQuantity])));
                _currentObject = null;
            };
            this.chk_all.selected = false;
            this.lbl_modifyMultipleError.visible = false;
        ...
        protected function onSearchTimerComplete(e:TimerEvent):void
        {
            ...
            {
                if (((((this.inp_search) && (this._searchCriteria.length <= 2)) && (!(this.inp_search.haveFocus))) && (!(this._searchCriteria == this.INPUT_SEARCH_DEFAULT_TEXT))))
                {
                    this._searchCriteria = null;
                    this.inp_search.placeholderText = this.INPUT_SEARCH_DEFAULT_TEXT;
                };
