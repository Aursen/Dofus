package Ankama_Tutorial.ui
{
    ...
    public class SubhintEditorUi 
    {
        ...
        {
            var input:Input;
            var arrowButton:ButtonContainer;
            var uiArr:Array;
            ...
            this.sysApi.addHook(SubhintEditorUpdated, this.onSubhintEditorUpdated);
        ...
        private function onUiUnloaded(uiName:String):void
        {
            ...
            {
                if (this.uiApi.getUi(this.lbl_uiNameDisplay.text.split("_")[0]))
                {
                    this.resetBaseData();
                };
            };
        ...
        private function onSubhintEditorUpdated(data:Object):void
        {
            this.subhintEditorUpdated(data, true);
        }

        public function subhintEditorUpdated(data:Object, withInspector:Boolean=false):void
        {
            ...
            if (withInspector)
            {
                ...
                {
                    if (this._createOrChange == "change")
                    {
                        if (data.hint_anchored_element.indexOf("btn_help") != -1)
                        {
                            this.cb_shPos.value = this._posChoices[0];
        ...
        private function updateInput(input:Input, increase:Boolean):void
        {
            ...
            if (increase)
            {
                ...
                {
                    if (this._currentInputToUpdate.maxChars == 2)
                    {
                        limitValue = 99;
                    }
                    else
                    {
                        if (this._currentInputToUpdate.maxChars == 3)
                        {
                            limitValue = 999;
                        }
                        else
                        {
                            if (this._currentInputToUpdate.maxChars == 4)
                            {
                                limitValue = 9999;
                            };
                        };
        ...
        public function onRelease(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                case this.btn_subhintList:
                    this.sysApi.sendAction(new OpenSubhintList());
                    break;
        ...
        {
            var currentWidthInput:int;
            var currentOrderInput:int;
            var pattern:RegExp;
            var input:Input;
            var myPattern:RegExp;
            switch (target)
            {
                case this.inp_ttWidth:
                    ...
                    {
                        if (currentWidthInput < this._minTtWidth)
                        {
                ...
                case this.inp_offsetY:
                    ...
                    {
                        if (input.text.match(new RegExp("-", "g")).length > 1)
                        {
        ...
        {
            var arrowButtonToDisabled:ButtonContainer;
        ...
        private function showCurrentSubhintInEditor():void
        {
            ...
            {
                if (!this.uiApi.getUiByName(this.lbl_uiNameDisplay.text.split("_")[0]))
                {
                    this.resetBaseData();
                };
            };
        ...
        private function compareAllParameters(op:String):void
        {
            ...
            {
                if (((!(this.currentSubhint)) || (op == "preview")))
                {
