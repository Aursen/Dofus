package Ankama_Cartography.ui
{
    ...
    public class CartographyUi extends CartographyBase 
    {
        ...
        private var _filterCat:Dictionary = new Dictionary(true);
        ...
        override protected function initMap():void
        {
            ...
            if (!__conquestSubAreasInfos)
            {
                if (modCartography.showConquestInformation())
                {
                    ...
                    sysApi.sendAction(new PrismsListRegister("cartography.Cartography", PrismListenEnum.PRISM_LISTEN_ALL));
        ...
        protected function updateAllFilters():void
        {
            ...
            for (btn in this._filterCat)
            {
                ...
                {
                    sysApi.dispatchHook(MapHintsFilter, id, btn.selected, true);
                };
            };
        ...
        {
            var list:AreaGroup;
            var selectedTerritoryTypeLayer:String;
            var itemValue:int;
        ...
        {
            var searchVisible:Boolean;
            var menu:Array;
            var breachWorldMapCoordinate:BreachWorldMapCoordinate;
            ...
            switch (target)
            {
                case this.btn_player:
                    ...
                    {
                        breachWorldMapCoordinate = ((breachApi.breachFrame) ? dataApi.getBreachWorldMapCoordinate(Math.max(dataApi.getBreachMinStageWorldMapCoordinate().mapStage, Math.min(breachApi.getFloor(), dataApi.getBreachMaxStageWorldMapCoordinate().mapStage))) : null);
        ...
        public function onKeyDown(target:Object, keyCode:uint):void
        {
            ...
            if ((((this.tiSearch.haveFocus) && (!(this._conquestMode))) && (this.gdSearchAll.dataProvider.length > 0)))
            {
                ...
                else
                {
                    ...
                    {
                        if ((((keyCode == Keyboard.ENTER) && (this._lastSearch)) && (this._searchCriteria == this._lastSearch)))
                        {
        ...
        public function onSelectItem(target:Object, method:uint, isNew:Boolean):void
        {
            ...
            var len:uint;
            var resourceQuantity:uint;
            var shapeColor:uint;
            var i:int;
            var subAreasIds:Object;
            var l:uint;
            var numSubAreas:int;
            var endIndex:int;
            var j:int;
                    ...
                    case ITEM_SEARCH_TYPE:
                        if (item.resourceSubAreaIds)
                        {
                            ...
                            while (i < len)
                            {
                                ...
                                this.showSearchSubArea(subAreaId, 0x6B6B6B, shapeColor, 1, 0.8);
                                i = (i + 2);
                            };
                        }
                        else
            ...
            {
                if (((target == this.gdSearchAll) && ((method == SelectMethodEnum.MANUAL) || (method == SelectMethodEnum.AUTO))))
                {
                ...
                {
                    if ((((item) && (item is AreaGroup)) && ((item.type == "areaShape") || (item.type == "dungeon"))))
                    {
        ...
        {
            var currentMapElement:String;
            var subArea:SubArea;
            var anomaly:Object;
            var coord:MapPosition;
            ...
            if ((((item.data) && (item.data.data)) && (item.data.type == "subarea")))
            {
                ...
                if (this._conquestMode)
                {
                    ...
                    {
                        alliance = ((prismInfo.alliance) ? prismInfo.alliance : socialApi.getAlliance());
            ...
            {
                if ((((item.data) && (item.data.data)) && (item.data.type == "dungeon")))
                {
        ...
        public function onItemRollOut(target:Object, item:Object):void
        {
            ...
            {
                if (__lastHighlightElement.indexOf("dungeon") != -1)
                {
                    mapElement = mapViewer.getMapElement(__lastHighlightElement);
                    if (mapElement)
                    {
        ...
        override public function onRollOver(target:Object):void
        {
            ...
            if (((!(target.name.indexOf("tx_itemType") == -1)) && (target.uri)))
            {
                ...
                {
                    if (target.uri.fileName == "success_cat_6.png")
                    {
        ...
        {
            var searchSubArea:SubArea;
            var hintSubArea:SubArea;
        ...
        private function searchAll(pSearch:String):void
        {
            ...
            if (((pSearch) && (pSearch.length > 0)))
            {
                ...
                this.gdSearchAll.height = ((height > uiApi.me().getConstant("gdSearchAll_max_height")) ? uiApi.me().getConstant("gdSearchAll_max_height") : height);
                ...
                this.ctr_search_bg.width = ((this.gdSearchAll.height == uiApi.me().getConstant("gdSearchAll_max_height")) ? this.gdSearchAll.width : this.gdSearchAll.slotWidth);
                if (this.gdSearchAll.dataProvider.length == 0)
                {
                    this.lbl_results.text = ((this._hasResultsInOtherWorldMap) ? uiApi.getText("ui.search.resultsInOtherWorldMap") : uiApi.getText("ui.search.noResult"));
                }
                else
                {
        ...
        private function getSubAreaInfoStr(pSubArea:SubArea, pSubAreaName:String):String
        {
            ...
            {
                if (((this._searchSelectedItem.type == ITEM_SEARCH_TYPE) && (this._searchSelectedItem.subAreasIds)))
                {
        ...
        private function showSearchSubArea(pSubAreaId:int, pLineColor:uint=0x333333, pFillColor:uint=1096297, pLineAlpha:Number=0.6, pFillAlpha:Number=0.4):void
        {
            var dungeon:Dungeon;
            ...
            else
            {
                ...
                {
                    if (!mapViewer.getMapElement(("search_shape" + pSubAreaId)))
                    {
        ...
        {
            var prismWrapper:PrismSubAreaWrapper;
            var prism:*;
            var hint:Object;
        ...
        private function highlightBreachZone(zoneId:int):void
        {
            ...
            var maxStage:int = ((sector.maxStage == 0) ? minStage : sector.maxStage);
            for each (mapElement in mapViewer.getMapElementsByLayer(MAP_LAYER_BREACH_HIGHLIGHT))
            {
                mapViewer.removeMapElement(mapElement);
            };
