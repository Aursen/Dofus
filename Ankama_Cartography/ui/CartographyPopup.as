    ...
    public class CartographyPopup extends AbstractCartographyUi 
    {
        ...
        private function initMap(pSelectedSubAreaId:int, pSubAreasIds:Object):void
        {
            ...
            if (dungeonId == -1)
            {
                ...
                {
                    mapViewer.addAreaShape(AREAS_SHAPES_LAYER, ("shape_" + selectedSubArea.id), mapApi.getSubAreaShape(selectedSubArea.id), 0xFF0000, 0.5, 0xFF0000, 0.5);
                };
            }
            else
            {
            ...
            this.addSubAreaPositionFlag(selectedSubArea.id, 0xFF0000);
            if (pSubAreasIds)
            {
                for each (subareaId in pSubAreasIds)
                {
