package Ankama_Job.ui
{
    ...
    public class Recycle 
    {
        ...
        private var _itemsToRecycle:Array = new Array();
        private var _recyclingSubareaIds:Array = new Array();
        ...
        private function prepareItem(itemWrapper:Object):Object
        {
            ...
            {
                bonus = int((Math.round(((nuggets / itemWrapper.nuggetsQuantity) * 100)) - 100));
            };
            ...
            if (target == this.gd_items)
            {
                ...
                switch (selectMethod)
                {
                    case SelectMethodEnum.DOUBLE_CLICK:
                        this.sysApi.sendAction(new ExchangeObjectMove(selectedItem.item.objectUID, -1));
                        break;
                    case SelectMethodEnum.CTRL_DOUBLE_CLICK:
                        this.sysApi.sendAction(new ExchangeObjectMove(selectedItem.item.objectUID, -(selectedItem.item.quantity)));
                        break;
        ...
        public function onRollOver(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                default:
                    ...
                    {
                        if (target.name.indexOf("lbl_areaBonus") != -1)
                        {
                            data = this._compInteractiveList[target.name];
