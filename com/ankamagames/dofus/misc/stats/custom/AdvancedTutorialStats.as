package com.ankamagames.dofus.misc.stats.custom
{
    ...
    public class AdvancedTutorialStats implements IStatsClass, IHookStats 
    {
        ...
        private static const TUTORIAL_STEP_CLICK_ON_MONSTERS:uint = 0x1900;
        ...
        private var _jobStepsHarvest:Array = new Array();
        ...
        public function process(pMessage:Message, pArgs:Array=null):void
        {
            ...
            {
                if ((pMessage is MapComplementaryInformationsDataMessage))
                {
                ...
                {
                    if ((pMessage is InteractiveUsedMessage))
                    {
                    ...
                    {
                        if ((pMessage is SelectItemMessage))
                        {
                        ...
                        {
                            if ((pMessage is GameFightStartingMessage))
                            {
                            ...
                            {
                                if ((pMessage is QuestValidatedMessage))
                                {
                                ...
                                {
                                    if ((pMessage is QuestListMessage))
                                    {
        ...
        public function onHook(pHook:Hook, pArgs:Array):void
        {
            ...
            var resourcesLen:uint;
            var entitiesFrame:RoleplayEntitiesFrame;
            var infos:GameRolePlayNpcInformations;
            var initStep:Boolean;
            var objective:*;
            switch (pHook.name)
            {
                case QuestHookList.QuestObjectiveValidated.name:
                    if (((pArgs[0] == QUEST_JOB_ID) || (pArgs[0] == QUEST_FIGHT_ID)))
                    {
                        if (this._currentStep == TUTORIAL_STEP_START_END_NPC_DIALOG)
                        {
                            ...
                            {
                                if (((!(this._stepInfos[stepId])) || (!(this._stepInfos[stepId].hasOwnProperty(FOLLOW_INSTRUCTIONS)))))
                                {
