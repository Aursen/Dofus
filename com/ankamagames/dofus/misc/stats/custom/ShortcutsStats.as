package com.ankamagames.dofus.misc.stats.custom
{
    ...
    public class ShortcutsStats implements IHookStats, IStatsClass 
    {
        ...
        public static const SHORTCUTS:Object = {
            ...
            "useSpellLine2":null,
            "openBreach":null,
            "openTemporisQuestUi":"questBase"
        };

        private var _usedShortcuts:Dictionary = new Dictionary();
        private var _shortcutsData:Dictionary = new Dictionary();
        ...
        public function onHook(pHook:Hook, pArgs:Array):void
        {
            ...
            {
                if (pHook.name == CustomUiHookList.FoldAll.name)
                {
                ...
                {
                    if (pHook.name == BeriliaHookList.UiLoading.name)
                    {
                    ...
                    {
                        if (pHook.name == BeriliaHookList.UiUnloaded.name)
                        {
        ...
        public function process(pMessage:Message, pArgs:Array=null):void
        {
            ...
            {
                if ((pMessage is MountToggleRidingRequestAction))
                {
                ...
                {
                    if ((pMessage is ExchangeRequestOnShopStockAction))
                    {
                        this.onNoUiShortcut("openSell");
                    }
                    else
                    {
                        if ((pMessage is OpenIdolsAction))
                        {
                            this.onNoUiShortcut("openIdols");
                        }
                        else
                        {
                            if ((pMessage is HavenbagEnterAction))
                            {
                                this.onToggleShortcut("openHavenbag");
                            }
                            else
                            {
                                if ((pMessage is ToggleDematerializationAction))
                                {
                                    this.onNoUiShortcut("toggleDematerialization");
                                }
                                else
                                {
                                    if ((pMessage is ShowAllNamesAction))
                                    {
                                        this.onNoUiShortcut("showAllNames");
                                    }
                                    else
                                    {
                                        if ((pMessage is ToggleEntityIconsAction))
                                        {
                                            this.onToggleShortcut("toggleEntityIcons");
                                        }
                                        else
                                        {
                                            if ((pMessage is ShowEntitiesTooltipsAction))
                                            {
                                                this.onToggleShortcut("showEntitiesTooltips");
                                            }
                                            else
                                            {
                                                if ((pMessage is HighlightInteractiveElementsAction))
                                                {
                                                    this.onToggleShortcut("highlightInteractiveElements");
                                                }
                                                else
                                                {
                                                    if ((pMessage is ShowFightPositionsAction))
                                                    {
                                                        this.onToggleShortcut("showFightPositions");
                                                    }
                                                    else
                                                    {
                                                        if ((pMessage is GameFightSpellCastAction))
                                                        {
                                                        ...
                                                        {
                                                            if (((!(this._action == null)) && (((pMessage is ChangeCharacterAction) || (pMessage is ChangeServerAction)) || (pMessage is ResetGameAction))))
                                                            {
