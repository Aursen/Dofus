package com.ankamagames.dofus.logic.game.fight.managers
{
    ...
    public class TacticModeManager 
    {
        ...
        private var _colorTransformByTileName:Dictionary = new Dictionary();
        ...
        public function show(pPoint:WorldPointWrapper, openByTerminal:Boolean=false):void
        {
            ...
            var canMov:Boolean;
            var canLos:Boolean;
            var nonWalkableDuringFight:Boolean;
            var nonWalkableDuringRp:Boolean;
            var tmpZonesList:Array;
            var o:Object;
            var pt:Point;
            var zonesMaxY:int;
            var zonesMinY:int;
            ...
            if (useNewTacticalMode)
            {
                ...
                this._templateToShow = ((OptionManager.getOptionManager("dofus").getOption("forceDefaultTacticalModeTemplate") == true) ? 1 : Math.max(templateId, 1));
            ...
            MapDisplayManager.getInstance().hideBackgroundForTacticMode(true, ((useNewTacticalMode) ? template.backgroundColor : 0));
            if (SHOW_BACKGROUND)
            {
                this.loadBackground();
            };
        ...
        private function clearUnneededCells(zonesMaxX:int, zonesMaxY:int, zonesMinX:int, zonesMinY:int):void
        {
            ...
            var centerCellId:int = int(CellIdConverter.coordToCellId(((avgX / 2) + zonesMinX), ((avgY / 2) + zonesMinY)));
