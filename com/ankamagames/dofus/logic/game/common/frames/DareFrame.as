package com.ankamagames.dofus.logic.game.common.frames
{
    ...
    public class DareFrame extends RegisteringFrame 
    {
        ...
        private function onDareListMessage(m:DareListMessage):Boolean
        {
            ...
            var oldHiddenIds:Array;
            var hiddenIds:Array;
            var hidden:String;
            var len:uint;
            var list:Vector.<DareInformations>;
            var i:uint;
            var dw2:DareWrapper;
            var endDate:Number;
            var oldDw:DareWrapper;
        ...
        private function onDareSubscribedMessage(m:DareSubscribedMessage):Boolean
        {
            ...
            var playedCharacterManager:PlayedCharacterManager;
            var playerInfo:Object;
            var dareIndex:int = -1;
            for each (dare in this._dareList)
            {
        ...
        private function onDareCreationRequest(a:DareCreationRequestAction):Boolean
        {
            ...
            var i:int;
            while (i < criteriaLength)
            {
                dc = new DareCriteria();
                _local_4 = a.criteria[i].shift();
                if ((((a.criteria[i]) && (a.criteria[i].length)) && (a.criteria[i][0])))
                {
                    dc.initDareCriteria(_local_4, a.criteria[i]);
        ...
        {
            var monterIds:Vector.<int>;
            var dareIdsInSubArea:Vector.<Number>;
            var subAreaMonsterKey:String;
            var subAreaId:uint;
            var dare:DareWrapper = this._dareListById[m.dareId];
