package com.ankamagames.dofus.logic.common.managers
{
    ...
    public class AbstractItemFilterManager 
    {
        ...
        protected var _currentItemWithSearchCriteria:Object = new Object();
        ...
        public function updateInputLevel(input:Input):void
        {
            ...
            {
                if (((!(input.name.indexOf("inp_maxLevel") == -1)) && (!(this._maxLevelInputValue == levelValue))))
                {
        ...
        public function displayCategories(selectedCategory:Object=null):Array
        {
            ...
            for each (entry in this._dataMatrix)
            {
                ...
                if (((!(entry.isCat)) && (!(this._openedCategories.indexOf(entry.cat) == -1))))
                {
                    ...
                    {
                        if (entry.cat == this.ITEMTYPE_CAT_ID)
                        {
                            tempCats = tempCats.concat(this.addItemTypeList(entry.itemType));
                        }
                        else
                        {
                            if (((entry.cat == this.LEVELFILTER_CAT_ID) && ((entry.id) && (entry.id == "level"))))
                            {
        ...
        {
            var obtainingId:uint;
            var id:*;
            var ids:Object = this.filterLevel();
        ...
        {
            var characId:uint;
            var mountWithEffect:Vector.<uint>;
            var effectId:uint;
            var weaponEffectId:uint;
            var consumableEffect:uint;
            var effectIds:Vector.<uint>;
            var mountWithCurrentEffect:Array;
            var id:uint;
            var mount:Mount;
            ...
            {
                if (this._currentCategory == this.CONSUMABLE_CATEGORY)
                {
                ...
                {
                    if (this._currentCategory == this.RESOURCE_CATEGORY)
                    {
        ...
        {
            this.setLevelRange(((levelMin > 0) ? levelMin : this._minLevel), ((levelMax > 0) ? levelMax : this._maxLevel));
        }
...
class FilterGridItem 
{
    ...
    public function FilterGridItem(gCid:int, gId:String, gText:String, gGfxId:String, gSelected:Boolean, gCat:int, typeId:int=-1)
    {
        this.cId = gCid;
        this.id = gId;
        this.text = gText;
