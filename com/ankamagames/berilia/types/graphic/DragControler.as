package com.ankamagames.berilia.types.graphic
{
    ...
    public class DragControler implements MessageHandler 
    {
        ...
        public var restrictionFunction:Function = restrictPosition;
        private var mousePositionOnMouseDown:Point = new Point();
        ...
        public function restrictPosition():void
        {
            ...
            if (!this._dragRestrictedArea.containsRect(this._currentBounds))
            {
                ...
                {
                    if (this._currentBounds.x > this._dragRestrictedArea.right)
                    {
                ...
                {
                    if (this._currentBounds.y > this._dragRestrictedArea.bottom)
                    {
        ...
        private function setupDragVars():void
        {
            ...
            var magneticElements:Array;
            var gc:GraphicContainer;
            var gcBounds:Rectangle;
        ...
        private function getMagneticResult(realBounds:Boolean=true):Rectangle
        {
            ...
            var biggestCollisionAreaSurface:uint;
            var biggestCollisionArea:MagnetGrid;
        ...
        private function getNewPosition(target:Rectangle, magneticArea:MagnetGrid):Rectangle
        {
            ...
            if (magneticArea.cornerX)
            {
                ...
                {
                    if (((((magneticArea.collidArea.right - target.x) - target.width) < this.MAGNET_LENGTH) && ((target.x + (target.width / 2)) < magneticArea.collidArea.right)))
                    {
                        result.x = (magneticArea.collidArea.right - target.width);
                    };
                };
            ...
            if (magneticArea.cornerY)
            {
                ...
                {
                    if (((((magneticArea.collidArea.bottom - target.y) - target.height) < this.MAGNET_LENGTH) && ((target.y + (target.height / 2)) < magneticArea.collidArea.bottom)))
                    {
                        result.y = (magneticArea.collidArea.bottom - target.height);
                    };
                };
        ...
        private function togglePreviewContainer(visible:Boolean):void
        {
            ...
            {
                if (((_previewContainer) && (_previewContainer.parent)))
                {
        ...
        {
            var magneticResult:Rectangle;
            var fullscreenOffset:Number;
...
class MagnetGrid 
{
    ...
    public function MagnetGrid(collidArea:Rectangle=null, cornerX:Boolean=true, cornerY:Boolean=true, magnetOnRightSide:Boolean=true, magnetOnLeftSide:Boolean=true, magnetOnTopSide:Boolean=true, magnetOnBottomSide:Boolean=true, name:String="")
    {
