package com.ankamagames.berilia.components
{
    ...
    public class TextFieldScrollBar extends GraphicContainer implements UIComponent 
    {
        ...
        private var linesLenghts:Dictionary = new Dictionary();
        private var _nbLines:Dictionary = new Dictionary();
        ...
        public function resize(nW:Number=0, nH:Number=0):void
        {
            ...
            x = ((this._leftSide) ? ((this._label.x - WIDTH) - 5) : (this._label.x + this._label.width));
            y = this._label.y;
            this.drawScrollBar();
        }
        ...
        {
            var oldMaxScroll:int;
            var oldMaxInterScroll:int;
            var nbCharDiffSelectionBegin:int;
            var newSelectionBegin:int;
            ...
            if (selectionBegin != selectionEnd)
            {
                ...
                addedLines = 0;
                i = oldMaxScroll;
                while (i < this._maxScroll)
                {
                    addedLines = (addedLines + this._nbLines[this._lines[i]]);
                    i++;
                };
                i = 0;
                while (i < (((addedLines + this._maxInterScroll) - oldMaxInterScroll) + this._interScroll))
                {
        ...
        public function removeLastText():void
        {
            ...
            if (((this._scroll == 0) && (!(selectionBegin == selectionEnd))))
            {
                nbCharDiffSelectionBegin = 0;
                i = 0;
                while (i < this._nbLines[removedText])
                {
                    nbCharDiffSelectionBegin = (nbCharDiffSelectionBegin + this._label.textfield.getLineLength(i));
                    i++;
                };
        ...
        private function onMouseWheel(mouseEvent:MouseEvent):void
        {
            ...
            var newSelectionEnd:int;
            var i:int;
            var alreadyScrolled:int;
            var initialInterScroll:int;
            var endIndex:int;
            if (!visible)
            {
                return;
            ...
            if (selectionBegin != selectionEnd)
            {
                ...
                {
                    i = 0;
                    while (i < (Math.min(this._power, this.remainingScrollableLines) + this._interScroll))
                    {
                        nbCharDiffSelectionBegin = (nbCharDiffSelectionBegin + this._label.textfield.getLineLength(i));
                        i++;
                    };
            ...
            if (selectionBegin != selectionEnd)
            {
                ...
                {
                    i = 0;
                    while (i < ((Math.min(this._power, alreadyScrolled) - initialInterScroll) + this._interScroll))
                    {
                        nbCharDiffSelectionBegin = (nbCharDiffSelectionBegin + this._label.textfield.getLineLength(i));
                        i++;
                    };
                    newSelectionBegin = (selectionBegin + nbCharDiffSelectionBegin);
                    newSelectionEnd = (selectionEnd + nbCharDiffSelectionBegin);
                    endIndex = (((this._numLines + this._interScroll) < this._label.textfield.numLines) ? (this._label.textfield.getLineOffset((this._numLines + this._interScroll)) - 1) : (this._label.textfield.text.length - 1));
