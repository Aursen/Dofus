package com.hurlant.crypto.symmetric
{
    ...
    public class DESKey implements ISymmetricKey 
    {
        ...
        protected function generateWorkingKey(encrypting:Boolean, key:ByteArray, off:uint):Array
        {
            ...
            {
                l = pc1[j];
                pc1m[j] = (!((key[(off + (l >>> 3))] & bytebit[(l & 0x07)]) == 0));
                j++;
            };
            var i:uint;
            while (i < 16)
        ...
        protected function desFunc(wKey:Array, inp:ByteArray, inOff:uint, out:ByteArray, outOff:uint):void
        {
            ...
            while (round < 8)
            {
                ...
                fval = SP7[(work & 0x3F)];
                fval = (fval | SP5[((work >>> 8) & 0x3F)]);
                fval = (fval | SP3[((work >>> 16) & 0x3F)]);
                fval = (fval | SP1[((work >>> 24) & 0x3F)]);
                work = (right ^ wKey[((round * 4) + 1)]);
                fval = (fval | SP8[(work & 0x3F)]);
                fval = (fval | SP6[((work >>> 8) & 0x3F)]);
                fval = (fval | SP4[((work >>> 16) & 0x3F)]);
                fval = (fval | SP2[((work >>> 24) & 0x3F)]);
                left = (left ^ fval);
                work = ((left << 28) | (left >>> 4));
