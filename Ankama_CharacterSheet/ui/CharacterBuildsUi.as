package Ankama_CharacterSheet.ui
{
    ...
    import d2actions.PresetDeleteRequest;
    ...
    import d2actions.ResetCharacterStatsRequest;
    import d2actions.CharacterPresetSaveRequest;
    ...
    public class CharacterBuildsUi 
    {
        ...
        private var _allSpellDisplayCache:Dictionary = new Dictionary();
        ...
        public function main(params:Object):void
        {
            ...
            this.sysApi.addHook(PlayedCharacterLookChange, this.updateLook);
            ...
            this.sysApi.addHook(PresetUsed, this.onPresetUsed);
        ...
        private function openBuildAtLaunch(buildId:int):void
        {
            var index:int;
            index = 0;
            while (index < this._builds.length)
            {
        ...
        public function unload():void
        {
            if (!this._isUnloading)
            {
                ...
                this.sysApi.dispatchHook(PreviewBuildSpellBar, this.storageApi.getShortcutBarContent(ShortcutBarEnum.SPELL_SHORTCUT_BAR), false);
            ...
            if (data)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            componentsRef.lbl_valueStat.text = this._characterCharacteristics[(data.keyword + "Current")];
                        }
                        else
                        {
        ...
        public function updateCaracLine(data:*, componentsRef:*, selected:Boolean):void
        {
            ...
            var truc:*;
            var isRoleplayOrPrefight:Boolean;
            var i:int;
            if (data)
            {
        ...
        public function updateCategory(data:*, componentsRef:*, selected:Boolean, line:uint):void
        {
            ...
            switch (this.getCatLineType(data, line))
            {
                ...
                case CTR_CAT_TYPE_ITEM:
                    ...
                    base = ((isNaN(currentBase)) ? (data.base + data.additionnal) : currentBase);
        ...
        public function updateSpellButtonsLine(data:*, componentsRef:*, selected:Boolean):void
        {
            ...
            if (data)
            {
                ...
                if (hasVariant)
                {
                    slotWidth = int((componentsRef.ctr_spellButtons.width / 2));
        ...
        private function updateSpellButton(spell:*, btn_spell:*, slot_icon:*, lbl_spellName:*):void
        {
            var spellCache:Object;
            var spellLevels:Array;
            var levelsCount:int;
            var i:int;
            var isFirstObtentionLevel:Boolean;
            var areAllObtentionLevelsTheSame:Boolean;
            var playerSpell:Object;
            if (spell)
            {
        ...
        public function equipObject(objectUID:int):void
        {
            if (this._equipmentPositionClicked != -1)
            {
                if (objectUID == -1)
                {
                    this.sysApi.sendAction(new ObjectSetPosition(this._equippedItemClicked, 63, 1));
                }
                else
                {
                    this.sysApi.sendAction(new ObjectSetPosition(objectUID, this._equipmentPositionClicked, 1));
                };
        ...
        private function createCharacteristicsDataMatrix():void
        {
            ...
            var categoryItem:CharacteristicGridItem;
            var charac:Characteristic;
            this._statsData = [];
        ...
        {
            var item:CharacteristicGridItem;
            var cloneItem:CharacteristicGridItem;
            var favItems:Array = [];
        ...
        {
            var totalVitality:int;
            var data:CharacteristicGridItem;
            var carac:Object;
            var maxLifePoints:int;
            var statsPoints:int;
            var caracBaseValue:int;
            var statDataInBuild:CharacterCharacteristicForPreset;
            ...
            var basePlayerCaracForceUse:Boolean;
            if (this._currentUiState == READ_ONLY_STATE)
            {
                ...
                for each (data in this._statsData)
                {
                    ...
                    else
                    {
                        ...
                        caracBaseValue = ((carac is CharacterBaseCharacteristic) ? carac.base : int(carac));
            ...
            var points:int = ((this._someStatsMayBeScrolled) ? (statsPoints + this._characterCharacteristics.additionnalPoints) : statsPoints);
        ...
        {
            var myIndex:int;
            var entry:CharacteristicGridItem;
            var scrollValue:int;
            var selecCatId:int;
            if (selectedCategory)
            {
        ...
        private function updateSpells():void
        {
            ...
            var spellInventory:Array;
            var spellsForPreset:Vector.<SpellForPreset>;
            var i:uint;
            var spellConversion:SpellConversion;
            var j:uint;
            var variantIds:Array;
            var breedSpellsIds:Array;
            var k:int;
            var lastElement:GraphicContainer;
            if (this._currentUiState == -1)
            {
                return;
            };
            ...
            {
                if (this._currentBuild.spellsPreset)
                {
                    spellsForPreset = this._currentBuild.spellsPreset.spells;
                    i = 0;
                    while (i < spellsForPreset.length)
                    {
                    ...
                    {
                        j = 0;
                        while (j < spellsForPreset.length)
                        {
        ...
        private function previewSpellBar():void
        {
            ...
            this.sysApi.dispatchHook(PreviewBuildSpellBar, spellsList, (this._currentUiState == READ_ONLY_STATE));
        }
        ...
        private function displayNewBuild():void
        {
            ...
            this._currentBuildNameHasChanged = false;
            var randomIconIndex:int = int(Math.floor((Math.random() * this.gd_buildIcons.dataProvider.length)));
        ...
        {
            var buildSpellsCount:int;
            var spell:Spell;
            var key:*;
            var buildItemWrapper:ItemWrapper;
            var characterItemWrapper:ItemWrapper;
            var upgradableStat:CharacteristicGridItem;
            var buildSpellShortcuts:Array;
            var spellBarShortcuts:Array;
            var spellConversion:SpellConversion;
            var spellData:Spell;
            var shortcutsArray:Vector.<int>;
            var sameItemFound:Boolean;
            ...
            while (i < buildEquipmentCount)
            {
                ...
                else
                {
                    sameItemFound = false;
                    j = 0;
                    while (j < characterEquipmentCount)
                    {
        ...
        private function equipBuild(buildId:int=-1):void
        {
            ...
            this.sysApi.sendAction(new PresetUseRequest(buildId));
        }

        private function deleteBuild():void
        {
            this.sysApi.sendAction(new PresetDeleteRequest(this._currentBuild.id));
        }
        ...
        {
            var temporisSpellsUiContainer:UiRootContainer;
            var isRoleplayOrPrefight:Boolean;
            var id:int;
            var superTypesForThisSlot:Array;
            var allItemsOfTheRightSuperType:Vector.<ItemWrapper>;
            var i:int;
            var itemsCount:int;
            var superType:int;
            ...
            var spellWrapper:SpellWrapper;
            var selectionAllowed:Boolean;
            var spellVariants:Array;
            var variantAvailableCount:int;
            var itsTheRightVariantsList:Boolean;
            var sw:SpellWrapper;
            ...
            switch (target)
            {
                ...
                case this.lbl_manageSpellSets:
                    ...
                    {
                        this.sysApi.sendAction(new OpenTemporisSpellsUi(true));
                    }
                    else
                    {
                        if (!this.uiApi.getUi(UIEnum.TEMPORIS_SPELL_SETS_UI))
                        {
                ...
                default:
                    if (this._currentUiState == MODIFY_STATE)
                    {
                        ...
                        if (((!(target.name.indexOf("btn_plus") == -1)) && (isRoleplayOrPrefight)))
                        {
                            ...
                            {
                                if (((this._componentList[target.name]) && (!(this.uiApi.getUi("statBoost").uiClass.statId == this._componentList[target.name].numId))))
                                {
                        ...
                        else
                        {
                            if (target.name.indexOf("slot_") == 0)
                            {
                                ...
                                {
                                    this.uiApi.hideTooltip();
                                    this.sysApi.dispatchHook(OpenItemsList, this._equippedItemClicked, allItemsOfTheRightSuperType, this.uiApi.getText("ui.common.equip"), this.equipObject);
                                };
                            }
                            else
                            {
                                if (((!(target.name.indexOf("btn_spell") == -1)) || (!(target.name.indexOf("slot_icon") == -1))))
                                {
                                    ...
                                    if (selectionAllowed)
                                    {
                                        this.sysApi.sendAction(new SpellVariantActivationRequest(spellWrapper.id));
                                    };
                                };
                            };
                        };
        ...
        public function onRollOver(target:Object):void
        {
            ...
            if (((target is Slot) && (target.data)))
            {
                ...
                this.uiApi.showTooltip(target.data, target, false, "standard", ((globalPosition.x > 500) ? LocationEnum.POINT_TOPRIGHT : LocationEnum.POINT_TOPLEFT), ((globalPosition.x > 500) ? LocationEnum.POINT_TOPLEFT : LocationEnum.POINT_TOPRIGHT), 20, null, null, settings);
                return;
            };
            if (((!(target.name.indexOf("btn_spell") == -1)) || (!(target.name.indexOf("slot_icon") == -1))))
            {
                ...
                default:
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (target.name.indexOf("lbl_bonus") != -1)
                                {
                                    data = this._componentList[target.name];
        ...
        public function onDoubleClick(target:Object):void
        {
            ...
            if (((((target is Slot) && (!((target as Slot).softDisabled))) && (target.data)) && (!(this._delayDoubleClickTimer.running))))
            {
                ...
                {
                    this.sysApi.sendAction(new ObjectSetPosition(target.data.objectUID, 63, 1));
                };
            };
        }
        ...
        {
            this.onPopupClose();
            this.sysApi.sendAction(new ResetCharacterStatsRequest());
        }
        ...
        public function onPopupSave():void
        {
            ...
            this.onPopupClose();
            var name:String = this.inp_buildName.text;
            var saveCharacterCurrentState:Boolean;
            ...
            this.sysApi.sendAction(new CharacterPresetSaveRequest(buildId, this._currentBuildIconId, name, saveCharacterCurrentState, SavablePresetTypeEnum.CHARACTER_BUILD));
        }
        ...
        {
            var buildOrEmptyTexture:Object;
            var currentBuildIndex:int;
            var buildCount:int;
            var buildWrapper:BuildWrapper;
            var newBuild:BuildWrapper;
            var buildsLogs:String = ("Mise à jour de la liste des presets  ou uniquement du preset " + buildId);
...
class CharacteristicGridItem 
{
    ...
    public function CharacteristicGridItem(id:int, keyword:String, name:String, gfxId:String="", numId:int=-1, upgradable:Boolean=false, categoryId:int=-1)
    {
        this.id = id;
