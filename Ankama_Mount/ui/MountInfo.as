package Ankama_Mount.ui
{
    ...
    public class MountInfo 
    {
        ...
        {
            var currentMapdId:Number;
            var mapIsUnderWater:Boolean;
            var mountFamilyId:int;
        ...
        public function showMountInformation(mount:Object, source:int):void
        {
            ...
            this.lbl_name.width = 0.48;
            this.lbl_name.x = 0.26;
            ...
            {
                if (mount.reproductionCount >= mount.reproductionCountMax)
                {
                    this.lbl_reproduction.cssClass = "red";
        ...
        private function onConfirmCutMount():void
        {
            if (this._paddockMode)
            {
                ...
                {
                    this.sysApi.sendAction(new ExchangeHandleMountStable(ExchangeHandleMountStableTypeEnum.EXCHANGE_EQUIPED_STERILIZE, [this._currentMount.id]));
                }
                else
                {
                    if (MountPaddock._currentSource == MountPaddock.SOURCE_BARN)
                    {
                        this.sysApi.sendAction(new ExchangeHandleMountStable(ExchangeHandleMountStableTypeEnum.EXCHANGE_MOUNTSTABLES_STERILIZE, [this._currentMount.id]));
                    }
                    else
                    {
                        if (MountPaddock._currentSource == MountPaddock.SOURCE_PADDOCK)
                        {
                            this.sysApi.sendAction(new ExchangeHandleMountStable(ExchangeHandleMountStableTypeEnum.EXCHANGE_MOUNTPADDOCK_STERILIZE, [this._currentMount.id]));
                        };
                    };
                };
            }
        ...
        private function onConfirmKillMount():void
        {
            if (this._paddockMode)
            {
                if (MountPaddock._currentSource == MountPaddock.SOURCE_EQUIP)
                {
                    this.sysApi.sendAction(new ExchangeHandleMountStable(ExchangeHandleMountStableTypeEnum.EXCHANGE_EQUIPED_FREE, [this._currentMount.id]));
                }
                else
                {
                    if (MountPaddock._currentSource == MountPaddock.SOURCE_INVENTORY)
                    {
                        this.sysApi.sendAction(new ExchangeHandleMountStable(ExchangeHandleMountStableTypeEnum.EXCHANGE_CERTIFICAT_FREE, [this._currentMount.id]));
                    }
                    else
                    {
                        if (MountPaddock._currentSource == MountPaddock.SOURCE_BARN)
                        {
                            this.sysApi.sendAction(new ExchangeHandleMountStable(ExchangeHandleMountStableTypeEnum.EXCHANGE_MOUNTSTABLES_FREE, [this._currentMount.id]));
                        }
                        else
                        {
                            if (MountPaddock._currentSource == MountPaddock.SOURCE_PADDOCK)
                            {
                                this.sysApi.sendAction(new ExchangeHandleMountStable(ExchangeHandleMountStableTypeEnum.EXCHANGE_MOUNTPADDOCK_FREE, [this._currentMount.id]));
                            };
                        };
                    };
                };
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            if (target == this.lbl_fecondation)
                                            {
                                                ...
                                                {
                                                    if (this._currentMount.isFecondationReady)
                                                    {
                                            ...
                                            {
                                                if (target.name.indexOf("lbl_cap") != -1)
                                                {
        ...
        public function onRelease(target:Object):void
        {
            switch (target)
            {
                ...
                case this.btn_mount:
                    ...
                    {
                        this.sysApi.sendAction(new MountToggleRidingRequest(true));
        ...
        private function initFeed():void
        {
            ...
            if ((((MountPaddock._currentSource == MountPaddock.SOURCE_EQUIP) || (MountPaddock._currentSource == MountPaddock.SOURCE_BARN)) && ((!(this.playerApi.isInFight())) || (this.playerApi.isInPreFight()))))
            {
                ...
                if (foodList.length)
                {
                    this.sysApi.dispatchHook(OpenMountFeed, this._currentMount.id, this._currentMount.model.familyId, MountPaddock._currentSource, foodList);
                }
                else
                {
