package Ankama_GameUiCore.ui
{
    ...
    import d2hooks.BreachMapInfos;
    import d2hooks.AnomalyMapInfos;
    import d2hooks.AnomalyState;
    import d2hooks.MapRewardRate;
    ...
    public class MapInfo 
    {
        ...
        public function main(... args):void
        {
            ...
            this.sysApi.addHook(BreachMapInfos, this.onBreachMapInfos);
            this.sysApi.addHook(AnomalyMapInfos, this.onAnomalyMapInfos);
            this.sysApi.addHook(AnomalyState, this.onAnomalyState);
            this.sysApi.addHook(MapRewardRate, this.onMapRewardRate);
        ...
        public function onPrismsList(pPrismsInfo:Object):void
        {
            ...
            if (!this._showAlliance)
            {
                ...
                {
                    this.showAllianceInfo(((prismSubAreaInformation.alliance) ? prismSubAreaInformation.alliance : this._myVeryOwnAlliance));
            ...
            for each (subAreaId in pPrismSubAreas)
            {
                if (this._currentSubAreaId == subAreaId)
                {
                    ...
                    if (prismSubAreaInfo.mapId != -1)
                    {
                        ...
                        this.showAllianceInfo(((prismSubAreaInfo.alliance) ? prismSubAreaInfo.alliance : this._myVeryOwnAlliance));
                        this._showAlliance = true;
                    }
                    else
                    {
        ...
        private function onBreachMapInfos(bosses:Vector.<MonsterInGroupLightInformations>):void
        {
            ...
            this.tx_allianceEmblemBack.visible = false;
            this.tx_allianceEmblemUp.visible = false;
            this.btn_availableDares.visible = false;
        }
        ...
        public function onMapComplementaryInformationsData(map:WorldPointWrapper, subAreaId:uint, show:Boolean):void
        {
            ...
            if (show)
            {
                if (!this.breachApi.isInBreach())
                {
                    ...
                    {
                        this.showAllianceInfo(((currentPrism.alliance) ? currentPrism.alliance : this._myVeryOwnAlliance));
                        this._showAlliance = true;
                    }
                    else
                    {
                    ...
                    this._totalDaresInSubArea = this.socialApi.getTotalDaresInSubArea(subAreaId);
        ...
        public function onRelease(target:Object):void
        {
            ...
            {
                subarea = this.dataApi.getSubArea(this._currentSubAreaId);
                if (subarea)
                {
                    area = subarea.area;
                    if (area)
                    {
                        searchString = (((area.name + " (") + subarea.name) + ")");
                    }
                    else
                    {
                        searchString = subarea.name;
                    };
                    this.sysApi.dispatchHook(OpenDareSearch, searchString, "searchFilterSubArea");
                }
                else
                {
        ...
        {
            var text:String;
            var mapBonusOrMalus:String;
            var subAreaBonusOrMalus:String;
            var timeLeft:Number;
            var pos:Object = {
            ...
            switch (target)
            {
                case this.btn_availableDares:
                    text = this.uiApi.processText(this.uiApi.getText("ui.social.dare.totalAvailable", this._totalDaresInSubArea), "", (this._totalDaresInSubArea == 1), (this._totalDaresInSubArea == 0));
                    break;
