package flashx.textLayout.events
{
    ...
    public class FlowElementMouseEventManager 
    {
        ...
        public function updateHitTests(xoffset:Number, clipRect:Rectangle, textFlow:TextFlow, startPos:int, endPos:int, container:ContainerController, needsCtrlKey:Boolean=false):void
        {
            ...
            var uniqueDictionary:Dictionary;
            var o:Object;
            var f:FlowElement;
            var interactiveObjects_LastTime:Array;
                ...
                for each (element in elements)
                {
                    ...
                    if (tf)
                    {
                        ...
                        for each (obj in elemRects)
                        {
                            ...
                            if (adjustLines)
                            {
                                width = ((container.measureWidth) ? clipRect.width : container.compositionWidth);
        ...
        tlf_internal function dispatchFlowElementMouseEvent(_arg_1:String, originalEvent:MouseEvent):Boolean
        {
            if ((((this._needsCtrlKey) && (!(originalEvent.ctrlKey))) && (!(_arg_1 == FlowElementMouseEvent.ROLL_OUT))))
            {
                return (false);
            };
            ...
            var textFlowListening:Boolean;
            if (textFlow)
            {
                textFlowListening = textFlow.hasEventListener(_arg_1);
            };
            if (((!(locallyListening)) && (!(textFlowListening))))
            {
                return (false);
            };
            var event:FlowElementMouseEvent = new FlowElementMouseEvent(_arg_1, false, true, this._currentElement, originalEvent);
            if (locallyListening)
            {
        ...
        private function localDispatchEvent(_arg_1:String, evt:MouseEvent):void
        {
            ...
            if (this.dispatchFlowElementMouseEvent(_arg_1, evt))
            {
                return;
            };
