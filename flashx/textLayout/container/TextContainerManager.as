package flashx.textLayout.container
{
    ...
    public class TextContainerManager extends EventDispatcher implements ISWFContext, IInteractionEventHandler, ISandboxSupport 
    {
        ...
        {
            return ((this._composeState == COMPOSE_FACTORY) ? this._damaged : this._textFlow.flowComposer.isPotentiallyDamaged(this._textFlow.textLength));
        }
        ...
        {
            var hadPreviousSelection:Boolean;
            ...
            if (hadPreviousSelection)
            {
                ...
                {
                    if (hasEventListener(SelectionEvent.SELECTION_CHANGE))
                    {
        ...
        {
            var controller:TMContainerController;
            ...
            this._composeState = (((textFlow.interactionManager) || (textFlow.mustUseComposer())) ? COMPOSE_COMPOSER : COMPOSE_FACTORY);
        ...
        private function invalidateInteractionManager(alwaysRecreateManager:Boolean=false):void
        {
            ...
            if (this._composeState == COMPOSE_COMPOSER)
            {
                ...
                activePos = ((interactionManager) ? interactionManager.activePosition : -1);
                anchorPos = ((interactionManager) ? interactionManager.anchorPosition : -1);
                if (this._editingMode == EditingMode.READ_ONLY)
                {
        ...
        public function compose():void
        {
            ...
            var inputManagerFactory:TextLineFactoryBase;
            var bounds:Rectangle;
            var firstObj:Object;
            var stringFactory:StringTextLineFactory;
            var format:TextLayoutFormat;
            var holderStyles:Object;
            var key:String;
            var val:*;
            var factory:TCMTextFlowTextLineFactory;
            if (this._composeState == COMPOSE_COMPOSER)
            {
        ...
        tlf_internal function factoryUpdateContainerChildren():void
        {
            ...
            if (Configuration.playerEnablesArgoFeatures)
            {
                ...
                idx = 0;
                while (idx < this._composedLines.length)
                {
        ...
        private function addActivationEventListeners():void
        {
            ...
            else
            {
                ...
                {
                    if (newState == HANDLERS_MOUSEWHEEL)
                    {
                        this._container.addEventListener(MouseEvent.MOUSE_WHEEL, this.mouseWheelHandler);
                    };
                };
        ...
        override public function dispatchEvent(event:Event):Boolean
        {
            ...
            {
                if (event.type == FlowOperationEvent.FLOW_OPERATION_BEGIN)
                {
...
class TMContainerController extends ContainerController 
{
    ...
    override protected function updateVisibleRectangle():void
    {
        ...
        {
            xpos = (xpos - ((isNaN(compositionWidth)) ? contentWidth : compositionWidth));
        };
