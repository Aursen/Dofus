package flashx.textLayout.edit
{
    ...
    public class SelectionManager implements ISelectionManager 
    {
        ...
        private static function computeSelectionIndexInContainer(textFlow:TextFlow, controller:ContainerController, localX:Number, localY:Number):int
        {
            ...
            var linePerpCoor:Number;
            var midPerpCoor:Number;
            var isLineBelow:Boolean;
            var prevPerpCoor:Number;
            var inPrevLine:Boolean;
            var textLine:TextLine;
            var startOnNextLineIfNecessary:Boolean;
            var lastAtom:int;
            var lastAtomRect:Rectangle;
            var lastLinePosInPar:int;
            var lastChar:String;
            var lineIndex:int = -1;
            ...
            else
            {
                ...
                if (!isTTB)
                {
                    ...
                    {
                        if (localX > (lastAtomRect.x + lastAtomRect.width))
                        {
                ...
                else
                {
                    ...
                    {
                        if (localY > (lastAtomRect.y + lastAtomRect.height))
                        {
                            if (startOnNextLineIfNecessary)
                            {
                                return ((textFlowLine.absoluteStart + textFlowLine.textLength) - 1);
        ...
        {
            var curCol:Rectangle;
            var nextCol:Rectangle;
            var colIdx:int;
        ...
        private static function findController(textFlow:TextFlow, target:Object, currentTarget:Object, localPoint:Point):ContainerController
        {
            ...
            while (containerIndex < textFlow.flowComposer.numControllers)
            {
                ...
                else
                {
                    ...
                    if (containerPoint.x < 0)
                    {
                        ...
                        {
                            if (containerPoint.y > containerHeight)
                            {
                    ...
                    {
                        if (containerPoint.x > containerWidth)
                        {
        ...
        tlf_internal static function computeSelectionIndex(textFlow:TextFlow, target:Object, currentTarget:Object, localX:Number, localY:Number):int
        {
            ...
            var controller:ContainerController;
            var rslt:int;
            var useTargetedTextLine:Boolean;
            if ((target is TextLine))
            {
        ...
        {
            var startCoords:CellCoordinates;
            var endCoords:CellCoordinates;
            if (table)
            {
        ...
        {
            var oldSelectionFormat:SelectionFormat;
            var newSelectionFormat:SelectionFormat;
        ...
        public function selectAll():void
        {
            ...
            {
                lastSelectablePos = ((this._textFlow.textLength > 0) ? (this._textFlow.textLength - 1) : 0);
        ...
        {
            var cell:TableCellElement;
            var cellCoords:CellCoordinates;
            var coords:CellCoordinates;
        ...
        public function mouseDoubleClickHandler(event:MouseEvent):void
        {
            ...
            var anchorPara:ParagraphElement;
            var anchorParaStart:int;
            if (!this.hasSelection())
            {
                return;
            };
            ...
            if (wmode != BlockProgression.RL)
            {
                ...
                if (cell)
                {
                    ...
                    if (useTableSelectionCursors)
                    {
                        ...
                        {
                            if (((((cell.rowIndex == 0) && (cell.colIndex == 0)) && (point.x < leftEdge)) && (point.y < topEdge)))
                            {
                                event.stopPropagation();
                                event.stopImmediatePropagation();
                            ...
                            {
                                if ((((cell.rowIndex == 0) && (point.x > leftEdge)) && (point.y < topEdge)))
                                {
                                    event.stopPropagation();
                                    event.stopImmediatePropagation();
