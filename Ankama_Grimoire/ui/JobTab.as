package Ankama_Grimoire.ui
{
    ...
    public class JobTab 
    {
        ...
        public function main(oParam:Object=null):void
        {
            ...
            var meatDisplayedAsSkill:Object;
        ...
        public function uiTutoTabLaunch(tab:String="btn_resources"):void
        {
            ...
            if (((this.hintsApi.canDisplayHelp()) && (this.hintsApi.guidedActivated())))
            {
                ...
                {
                    if (!this.hintsApi.getGuidedAlreadyDone(tab))
                    {
        ...
        public function updateJobLine(data:*, componentsRef:*, selected:Boolean):void
        {
            ...
            if (data)
            {
                ...
                {
                    xpPos = int(int((((data.jobXP - data.jobXpLevelFloor) / (data.jobXpNextLevelFloor - data.jobXpLevelFloor)) * 100)));
                    if (xpPos == 0)
                    {
                        xpPos = 1;
                    };
        ...
        {
            var skill:Skill;
            var meatsHuntableAtCurrentLevel:Array;
            var meat:Object;
            ...
            var i:int = 1;
            while (i <= this._currentJob.jobLevel)
            {
                levelData[(i - 1)] = {
                    "label":i.toString(),
                    "value":i
        ...
        public function onRelease(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                case this.chk_freeOption:
                    this.sysApi.sendAction(new JobCrafterDirectoryDefineSettings(this._currentJob.id, this.cbx_minLevelOption.selectedItem.value, this.chk_freeOption.selected));
                    break;
                case this.btn_close:
                ...
                case this.chk_subscribeAll:
                    ...
                    this.sysApi.sendAction(new JobBookSubscribeRequest(jobIds));
                    break;
            ...
            if (target.name.indexOf("slot") == 0)
            {
                ...
                {
                    this.sysApi.dispatchHook(ShowObjectLinked, data);
                };
            }
            else
            {
                ...
                {
                    if (target.name.indexOf("lbl_info") == 0)
                    {
                        data = this._gridComponentsList[target.name];
                        ...
                        this.sysApi.dispatchHook(OpenEncyclopedia, "bestiaryTab", bestiaryData);
                    };
                };
            };
        }
        ...
        public function onRollOver(target:Object):void
        {
            ...
            {
                if (target.name.indexOf("tx_jobLevelHelp") == 0)
                {
                    ttData = this.uiApi.getText("ui.craft.jobLevelHelp");
                }
                else
                {
                    if (target == this.chk_subscribeAll)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            if (target == this.chk_optionSubscribe)
                            {
                                if (this._currentJob)
                                {
        ...
        private function onJobBookSubscription(jobId:uint, subscriber:Boolean):void
        {
            ...
            {
                if ((((!(this._allJobsSubscribed)) && (subscriber)) && (allOtherJobsSubscribed)))
                {
                    this.chk_subscribeAll.selected = true;
                    this._allJobsSubscribed = true;
                };
