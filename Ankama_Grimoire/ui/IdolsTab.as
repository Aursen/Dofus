package Ankama_Grimoire.ui
{
    ...
    import d2actions.IdolsPresetSaveRequest;
    ...
    public class IdolsTab 
    {
        ...
        public function main(oParam:Object=null):void
        {
            ...
            this.sysApi.addHook(IdolsPresetDelete, this.onIdolsPresetDelete);
            ...
            this.sysApi.addHook(DropEnd, this.onIdolsDropEnd);
        ...
        public function updateIdolLine(idolData:Object, components:*, selected:Boolean):void
        {
            ...
            var numIcons:uint;
            var i:int;
            var incompatibleMonstersNames:Vector.<String>;
            var incompatibleMonster:Monster;
            var incompatibleMonsterId:int;
            var owners:String;
            var id:Number;
            ...
            if (this._forceLineRollOver)
            {
                ...
                {
                    if ((((idolData) && (!(idolData.readOnly))) && ((this._activeIdolsIds.length < MAX_ACTIVE_IDOLS) || (!(this._activeIdolsIds.indexOf(idolData.idolId) == -1)))))
                    {
        ...
        {
            var incompatibleMonstersIds:Vector.<int>;
            var incompatibleMonstersNames:Vector.<String>;
            var incompatibleMonsterId:int;
            var idol:Idol;
            var idolId:int;
        ...
        private function selectIdol(pIdolId:uint, pActivate:Boolean, pGrid:Grid):void
        {
            ...
            this.sysApi.sendAction(new IdolSelectRequest(pIdolId, pActivate, (!(this.btn_solo.selected))));
        }
        ...
        private function enableIdol(pIdolId:uint, pParty:Boolean, index:int=-1):void
        {
            ...
            var activeIdols:Vector.<ItemWrapper> = ((pParty) ? this._currentActiveGroupIdols : this._currentActiveSoloIdols);
        ...
        private function updateTotals():void
        {
            ...
            var idol:Idol;
            var coeff:Number;
            var totalScoreWithoutCoeff:uint;
            var incompatibleMonstersIds:Vector.<int>;
            var incompatibleMonstersNames:Vector.<String>;
            var incompatibleMonsterId:int;
            var incompatibleMonster:Monster;
            ...
            this.tx_synergy.visible = ((this.btn_presets.selected) ? null : ((this.hasSynergies(true)) || (this.hasSynergies(false))));
            for each (incompatibleMonsterId in incompatibleMonstersIds)
            {
                incompatibleMonster = this.dataApi.getMonsterFromId(incompatibleMonsterId);
                if (incompatibleMonster)
        ...
        private function getSynergiesText(pSynergiesList:Dictionary, pPositiveSynergies:Boolean):String
        {
            var idolId:*;
            var idol:Idol;
            var synergies:Vector.<IdolSynergy>;
        ...
        private function filter(pDataFieldName:String, pDataFieldValue:*, pOperator:String="=="):void
        {
            ...
            var numIdols:uint = ((this._currentDataProvider) ? this._currentDataProvider.length : 0);
            if (numIdols > 0)
            {
        ...
        private function deletePreset():void
        {
            ...
            {
                this.sysApi.sendAction(new PresetDeleteRequest(this._presetToDelete.presetId));
            };
        }
        ...
        private function savePreset(pPreset:Object):void
        {
            ...
            presetScore = 0;
            i = 0;
            while (i < pPreset.idolsIds.length)
            {
        ...
        private function initButtonValue(pValueName:String, pButton:ButtonContainer, pDefaultValue:Boolean):void
        {
            ...
            pButton.selected = ((selected != null) ? selected : pDefaultValue);
            ...
            if (((partyLeaderUpdate) || (((nextSelection) && (nextSelection.idolId == pIdolId)) && (nextSelection.activate == pActivate))))
            {
                ...
                else
                {
                    if (nextSelection)
                    {
                        ...
                        {
                            if (nextSelection.target == this.gd_idols)
                            {
                ...
                if (forceRollOver)
                {
                    idolIndex = int((this.gd_idols.getIndex() + Math.floor((this.gd_idols.mouseY / this.gd_idols.slotHeight))));
        ...
        public function onGameFightEnd(pResults:Object):void
        {
        ...
        public function onRelease(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                case this.btn_savePreset:
                    ...
                    this.sysApi.sendAction(new IdolsPresetSaveRequest(-1, this._currentIcon));
                    break;
        ...
        public function onRollOver(target:Object):void
        {
            var txt:String;
            var owners:String;
            var globalSynergyColor:String;
            var globalSynergy:String;
            var positiveSynergies:Boolean;
            var totalExp:uint;
            var totalLoot:uint;
            var numPresetIdols:uint;
            var idol:Idol;
            var coeff:Number;
            var i:int;
            var idolId:int;
            var selectedOnlyIdols:Boolean;
            var positiveSynergiesText:String;
            var negativeSynergiesText:String;
            ...
            else
            {
                ...
                {
                    if (target.name.indexOf("tx_memberIdol") != -1)
                    {
                    ...
                    {
                        if (target.name.toLowerCase().indexOf("incompatiblemonsters") != -1)
                        {
                        ...
                        {
                            if (target.name.indexOf("lbl_exp") != -1)
                            {
                                txt = this.uiApi.getText("ui.idol.tooltip.bonusExp");
                            }
                            else
                            {
                                if (target.name.indexOf("lbl_loot") != -1)
                                {
                                    txt = this.uiApi.getText("ui.idol.tooltip.bonusLoot");
                                }
                                else
                                {
                                    if (target.name.indexOf("tx_synergy") != -1)
                                    {
                                    ...
                                    {
                                        if (target.name.indexOf("slot_icon_m_gd_presets") != -1)
                                        {
                                            numPresetIdols = target.data.idolsIds.length;
                                            i = 0;
                                            while (i < numPresetIdols)
                                            {
                                        ...
                                        {
                                            if (((!(target.name.indexOf("lbl_idol_score") == -1)) && (this.btn_showSynergyScore.selected)))
                                            {
        ...
        public function onChange(target:Input):void
        {
            switch (target)
            {
                case this.inp_search:
                    ...
                    {
                        if (this.gd_idols.dataProvider != this._filteredDataProvider)
                        {
                            this.gd_idols.dataProvider = this._filteredDataProvider;
                        };
                    };
        ...
        {
            this.sysApi.sendAction(new PresetUseRequest(target.data.id));
        }
...
class IdolSelection 
{
    ...
    public function IdolSelection(pIdolId:uint, pItemId:uint, pActivate:Boolean, pTarget:Object, pSelectedIndex:int)
    {
        this.idolId = pIdolId;
        this.itemId = pItemId;
...
class IdolSynergy 
{
    ...
    public function IdolSynergy(pIdolId:uint, pSynergyCoeff:Number)
    {
        this.idolId = pIdolId;
