package Ankama_Grimoire.ui
{
    ...
    public class GameplayClassWindow 
    {
        ...
        private function createMecanic():void
        {
            ...
            for each (node in formatedDescription.children())
            {
                ...
                if (((nodeName == TITLE) || (nodeName == SUBTITLE)))
                {
                    if (desc != "")
                    {
                        groupPos = (groupPos + (((oldNodeName == TITLE) || (oldNodeName == SUBTITLE)) ? 0 : marginY));
                    ...
                    {
                        if ((iconId = parseInt(node.attribute("iconStateId"))) > 0)
                        {
        ...
        private function addTitle(title:String, node:XML, iconId:int, nodeName:String, marginY:int, oldNodeName:String, groupPos:int):void
        {
            ...
            labelTitle.x = ((iconId <= 0) ? ((nodeName == SUBTITLE) ? marginY : this.tx_mecanicIcon.x) : this.lbl_mecanicTitle.x);
            labelTitle.y = ((oldNodeName == "base") ? this.lbl_mecanicTitle.y : ((this.lbl_mecanicTitle.y + groupPos) + (((oldNodeName == TITLE) && (!(nodeName == SUBTITLE))) ? 0 : marginY)));
        ...
        private function addIcon(posIcon:Vector3D, oldNodeName:String, groupPos:int, marginY:int, iconUri:Uri):void
        {
            ...
            iconTitle.y = ((oldNodeName == "base") ? posIcon.y : ((posIcon.y + groupPos) + marginY));
