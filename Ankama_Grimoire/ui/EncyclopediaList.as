package Ankama_Grimoire.ui
{
    ...
    public class EncyclopediaList 
    {
        ...
        private var _allTypes:Dictionary = new Dictionary(true);
        ...
        public function onUiLoaded(uiName:String):void
        {
            if (uiName == this.uiApi.me().name)
            {
                ...
                {
                    if (((this._params.hasOwnProperty("forceOpenResourceTab")) && (this._params.forceOpenResourceTab)))
                    {
                    ...
                    {
                        if (((this._params.hasOwnProperty("forceOpenConsumableTab")) && (this._params.forceOpenConsumableTab)))
                        {
                            this.inp_search.text = this._params.resourceSearch.toLowerCase();
                            this.gd_list.dataProvider = [this.parseItem(this._params.resourceId)];
                        }
                        ...
                        {
                            if (((this._params.hasOwnProperty("forceOpenEquipmentTab")) && (this._params.forceOpenEquipmentTab)))
                            {
                                this.inp_search.text = this._params.resourceSearch.toLowerCase();
                                this.gd_list.dataProvider = [this.parseItem(this._params.resourceId)];
                            };
        ...
        public function updateCategory(data:*, componentsRef:*, selected:Boolean, line:uint):void
        {
            switch (this.getCatLineType(data, line))
            {
                case CTR_SEARCH:
                    ...
                    {
                        if (((((!(this._searchCriteria)) && (this.inp_search.text == "")) && (!(this.inp_search.haveFocus))) || (this.inp_search.text == this.INPUT_SEARCH_DEFAULT_TEXT)))
                        {
        ...
        private function parseItem(itemId:uint):Object
        {
            ...
            if (((!(tmpItem.hasOwnProperty("etheral"))) || (!(tmpItem.etheral))))
            {
                ...
                {
                    if (tmpItem.dropMonsterIds.length > 0)
                    {
                        item.obtaining.push({
                            "id":2,
                            "item":item.itemWrapper,
        ...
        {
            var sortedIds:Object;
            var id:uint;
            var j:uint;
            var itemTypesSorted:Vector.<uint>;
            var typeId:uint;
            var allTypeIds:Array;
            var itemId:uint;
            var key:*;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (target.name.indexOf("tx_obtaining") != -1)
                            {
                                ...
                                {
                                    this.sysApi.dispatchHook(OpenRecipe, obtainigData.item);
                                }
                                else
                                {
                                    if (obtainigData.id == ITEM_DROPABLE_CAT_ID)
                                    {
                                        itemData = new Object();
                                        ...
                                        this.sysApi.dispatchHook(OpenEncyclopedia, "bestiaryTab", itemData);
                                    }
                                    else
                                    {
                                        if (obtainigData.id == ITEM_HARVESTABLE_CAT_ID)
                                        {
