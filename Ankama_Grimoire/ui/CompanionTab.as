package Ankama_Grimoire.ui
{
    ...
    public class CompanionTab 
    {
        ...
        private var _colorDisable:ColorTransform = new ColorTransform(1, 1, 1, 0.4);
        private var _coloravailable:ColorTransform = new ColorTransform();
        private var _allCompanions:Array = new Array();
        private var _myCompanions:Array = new Array();
        ...
        {
            var compCarac:CompanionCharacteristic;
            var carac:Characteristic;
            var caracId:int;
                ...
                if (myCompanionOfThisType.isEthereal)
                {
                    for each (effect in myCompanionOfThisType.item.effects)
                    {
                        if (effect.effectId == ActionIds.ACTION_ITEM_CHANGE_DURABILITY)
                        {
                            ...
                            resPos = uint(int(((diceNum / effect.value) * 100)));
        ...
        private function getStatValue(pStatPerLevelRange:Object, pLevel:int):int
        {
            ...
            var i:int;
            var j:int;
            var levelDone:int = ((pStatPerLevelRange[i][0] !== 1) ? 1 : 0);
            while (i < numValues)
            {
                j = levelDone;
                while (j < pStatPerLevelRange[i][0])
                {
                    if (j < pLevel)
                    {
            ...
            switch (target)
            {
                ...
                case this.btn_equip:
                    ...
                    if (myComp)
                    {
                        ...
                        {
                            this.sysApi.sendAction(new ObjectSetPosition(myComp.item.objectUID, CharacterInventoryPositionEnum.INVENTORY_POSITION_ENTITY, 1));
                        };
                    };
                    break;
                case this.btn_help:
