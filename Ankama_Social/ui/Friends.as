package Ankama_Social.ui
{
    ...
    import d2hooks.ChatServiceFriendListUpdated;
    import d2hooks.ChatServiceUserFriendCreated;
    import d2hooks.ChatServiceUserFriendDeleted;
    import d2hooks.ChatServiceFriendInviteListUpdated;
    import d2hooks.ChatServiceFriendInviteCreated;
    import d2hooks.ChatServiceFriendInviteProcessed;
    import d2hooks.ChatServiceUserUpdatedPresence;
    import d2hooks.ChatServiceUserUpdatedStatus;
    import d2hooks.ChatServiceUserUpdatedActivities;
    ...
    import d2hooks.ShareStatusState;
    ...
    import d2hooks.ChatServiceUserUpdateHisActivity;
    ...
    public class Friends 
    {
        ...
        private var _tabsSortType:Dictionary = new Dictionary(true);
        private var _componentData:Dictionary = new Dictionary(true);
        private var _dictionnaryFriends:Dictionary = new Dictionary(true);
        ...
        public function main(params:Array):void
        {
            ...
            this.sysApi.addHook(ChatServiceFriendListUpdated, this.onFriendLauncherListUpdated);
            this.sysApi.addHook(ChatServiceUserFriendCreated, this.onFriendLauncherCreated);
            this.sysApi.addHook(ChatServiceUserFriendDeleted, this.onFriendLauncherDeleted);
            this.sysApi.addHook(ChatServiceFriendInviteListUpdated, this.onPendingInvitationsUpdated);
            this.sysApi.addHook(ChatServiceFriendInviteCreated, this.onFriendInviteLauncherCreated);
            this.sysApi.addHook(ChatServiceFriendInviteProcessed, this.onFriendInviteLauncherDeleted);
            this.sysApi.addHook(ChatServiceUserUpdatedPresence, this.onUserUpdatedPresence);
            this.sysApi.addHook(ChatServiceUserUpdatedStatus, this.onUserUpdateStatus);
            this.sysApi.addHook(ChatServiceUserUpdatedActivities, this.onUserUpdateActivities);
            ...
            this.sysApi.addHook(ShareStatusState, this.onStatusShareState);
            ...
            this._nCurrentTab = (((!(params == null)) && (params.length > 0)) ? params[0] : 0);
        ...
        {
            var itemCategory:Friends_Category_Item;
            var showFriendsElem:Boolean;
            var friendLauncher:Friend;
            var accountId:uint;
            var character:SocialCharacterWrapper;
            var completeInfos:Boolean;
            var isClosed:Boolean;
            var userProperties:Object;
            var gameActivities:Dictionary;
            ...
            switch (this.getCategoryLineType(data, line))
            {
                ...
                case CTR_FRIEND:
                    ...
                    if ((((friendLauncher.presence == UserPresence.ONLINE) && (data.activities)) && (data.activities.length > 0)))
                    {
                        ...
                        {
                            if (userProperties.applicationId)
                            {
        ...
        private function setFriendsInfo(friend:FriendWrapper, componentsRef:*):void
        {
            ...
            var ariw:ArenaRankInfosWrapper;
            var smiley:Smiley;
        ...
        private function displaySelectedTab(tab:uint):void
        {
            ...
            switch (tab)
            {
                case 0:
                    ...
                    this.btn_openWait.y = ((isWaitListOpen) ? POSY_REDUCED_BTN_OPENWAIT : POSY_EXPANDED_BTN_OPENWAIT);
                    this.gd_friends.height = ((isWaitListOpen) ? HEIGHT_REDUCED_FRIENDS_GRID : HEIGHT_EXPANDED_FRIENDS_GRID);
        ...
        private function hasPendingInvitationsReceived():Boolean
        {
            ...
            var i:int;
            while (i < this.gd_pendingInvitations.dataProvider.length)
            {
        ...
        {
            var category:Friends_Category_Item;
            var entry:Object;
            var scrollValue:int;
            ...
            for each (entry in this._dataMatrix)
            {
                ...
                {
                    if (((entry is Friend) || (this._closedCategories.indexOf(entry.e_category) == -1)))
                    {
                        tempCats.push(entry);
                    };
                };
        ...
        private function sortGrid(category:String=null):void
        {
            ...
            if (this._currentSortElem != null)
            {
                ...
                if (count > 0)
                {
                    ...
                    tempArray = tempArray.sort(this._currentSortElem.sortFunc, ((this._currentSortElem.ascending) ? 0 : Array.DESCENDING));
                    index = 0;
        ...
        private function sortByGameForActivities(firstItem:Friend, secondItem:Friend):int
        {
            ...
            var firstValue:int = ((firstActivity) ? firstItem.activities[0].applicationId : GREAT_VALUE);
            var secondValue:int = ((secondActivity) ? secondItem.activities[0].applicationId : GREAT_VALUE);
            if (firstValue > secondValue)
            {
        ...
        private function sortByGameForServer(firstItem:Friend, secondItem:Friend):int
        {
            var firstActivity:EndpointProperties = this.getFriendActivities(firstItem);
            var secondActivity:EndpointProperties = this.getFriendActivities(secondItem);
            var firstValue:int = ((firstActivity) ? firstItem.activities[0].applicationId : 1000);
            var secondValue:int = ((secondActivity) ? secondItem.activities[0].applicationId : 1000);
            if (firstValue > secondValue)
            {
                return ((this._currentSortElem.ascending) ? 1 : -1);
            };
        ...
        {
            var contact:SocialCharacterWrapper;
            this._contactsList = (this.socialApi.getContactsList() as Vector.<SocialCharacterWrapper>);
            this._friendsList = (this.socialApi.getFriendsList() as Vector.<SocialCharacterWrapper>);
        ...
        public function onSelectItem(target:Object, selectMethod:uint, isNewSelection:Boolean):void
        {
            switch (target)
            {
                case this.cbx_activity:
                    this.sysApi.dispatchHook(ChatServiceUserUpdateHisActivity, this.cbx_activity.selectedIndex);
        ...
        public function onRelease(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                case this.btn_shareStatus:
                    this.sysApi.sendAction(new StatusShareSet((!(this.btn_shareStatus.selected))));
                    break;
                ...
                default:
                    ...
                    if (target.name.indexOf("btn_unlock") != -1)
                    {
                        ...
                        {
                            if (((((!(data is Friend)) && (data.e_category == SocialCharacterCategoryEnum.CATEGORY_FRIEND)) || (data.e_category == SocialCharacterCategoryEnum.CATEGORY_CONTACT)) && (this._contactIdWaitingForKick == -1)))
                            {
                            ...
                            {
                                if (((data.e_category == SocialCharacterCategoryEnum.CATEGORY_ENEMY) && (this._enemyIdWaitingForKick == -1)))
                                {
                                ...
                                {
                                    if (data.e_category == SocialCharacterCategoryEnum.CATEGORY_IGNORED)
                                    {
                    ...
                    {
                        if (target.name.indexOf("btn_addToList") != -1)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                if (target.name.indexOf("tx_havenbag") != -1)
                                {
                                    this.sysApi.sendAction(new HavenbagEnter(data.playerId));
                                }
                                else
                                {
                                    if (((!(target.name.indexOf(CTR_CAT) == -1)) && (!(this._nCurrentTab == 0))))
                                    {
                                        this.displayCategories(data.e_category, data.count);
                                    }
                                    else
                                    {
                                        if (target.name.indexOf("btn_sort_friend_account") != -1)
                                        {
                                            this.sortGridWithCat(this.sortByAccountName, "account");
                                        }
                                        else
                                        {
                                            if (target.name.indexOf("btn_sort_friend_activity") != -1)
                                            {
                                                this.sortGridWithCat(this.sortByGameForActivities, "activity");
                                            }
                                            else
                                            {
                                                if (target.name.indexOf("btn_sort_friend_server") != -1)
                                                {
                                                    this.sortGridWithCat(this.sortByGameForServer, "server");
                                                }
                                                else
                                                {
                                                    if (target.name.indexOf("btn_tabGuild") != -1)
                                                    {
                                                        this.sortGridWithCat(this.sortByGuildName, "guild");
                                                    }
                                                    else
                                                    {
                                                        if (target.name.indexOf("btn_tabName") != -1)
                                                        {
                                                            this.sortGridWithCat(this.sortByName, "name");
                                                        }
                                                        else
                                                        {
                                                            if (target.name.indexOf("btn_tabLevel") != -1)
                                                            {
                                                                this.sortGridWithCat(this.sortByLevel, "level");
                                                            }
                                                            else
                                                            {
                                                                if (target.name.indexOf("btn_tabBreed") != -1)
                                                                {
                                                                    this.sortGridWithCat(this.sortByBreed, "breed");
                                                                }
                                                                else
                                                                {
                                                                    if (target.name.indexOf("btn_tabAchievement") != -1)
                                                                    {
                                                                        this.sortGridWithCat(this.sortByAchievement, "achievement");
                                                                    }
                                                                    else
                                                                    {
                                                                        if (target.name.indexOf("btn_tabLeague") != -1)
                                                                        {
                                                                            this.sortGridWithCat(this.sortByLeague, "league");
                                                                        }
                                                                        else
                                                                        {
                                                                            if (target.name.indexOf("btn_tabState") != -1)
                                                                            {
                                                                                this.sortGridWithCat(this.sortByState, "state");
                                                                            }
                                                                            else
                                                                            {
                                                                                if (target.name.indexOf("btn_acceptInvitation") != -1)
                                                                                {
                                                                                    this.acceptInvitation(data);
                                                                                }
                                                                                else
                                                                                {
                                                                                    if (target.name.indexOf("btn_refuseInvitation") != -1)
                                                                                    {
                                                                                        this.refuseInvitation(data);
                                                                                    }
                                                                                    else
                                                                                    {
                                                                                        if (target.name.indexOf("btn_cancelInvitation") != -1)
                                                                                        {
                                                                                            this.cancelInvitation(data);
                                                                                        }
                                                                                        else
                                                                                        {
                                                                                            if (target.name.indexOf("btn_addAnkamaFriend") != -1)
                                                                                            {
                                                                                                this.showAddFriendUI(SocialCharacterCategoryEnum.CATEGORY_FRIEND_ANKAMA, data.name);
                                                                                            }
                                                                                            else
                                                                                            {
                                                                                                if (target.name.indexOf("btn_addContact") != -1)
                                                                                                {
                                                                                                    this.sysApi.sendAction(new AddFriend(data.user.name));
                                                                                                };
                                                                                            };
                                                                                        };
                                                                                    };
        ...
        public function onRollOver(target:Object):void
        {
            ...
            {
                if (target.name.indexOf("tx_breed") != -1)
                {
                    data = this._componentData[target.name];
                ...
                else
                {
                    ...
                    {
                        if (target.name.indexOf("tx_league") != -1)
                        {
                        ...
                        else
                        {
                            if (target.name.indexOf("lbl_name") != -1)
                            {
                                ...
                                if (((data) && (!(data.online))))
                                {
                                    ...
                                    {
                                        months = int(Math.floor((data.lastConnection / 720)));
                                        days = int(((data.lastConnection - (months * 720)) / 24));
                                        if (months > 0)
                                        {
                                            if (days > 0)
                                            {
                            ...
                            {
                                if (target.name.indexOf("tx_game") != -1)
                                {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (((!(target.name.indexOf("tx_level") == -1)) || (!(target.name.indexOf("lbl_level") == -1))))
                                        {
        ...
        {
            this.sysApi.sendAction(new RemoveFriend(this._contactIdWaitingForKick));
...
final class Friends_Category_Item 
{
    ...
    public function Friends_Category_Item(name:String, e_category:uint, count:uint)
    {
        this.name = name;
...
final class Friends_Sort_Item 
{
    ...
    public function Friends_Sort_Item(e_category:uint)
    {
        this.e_category = e_category;
    }
...
final class SortType 
{
    ...
    public function SortType(_sortFunc:Function, _ascending:Boolean=true, _targetName:String="name")
    {
