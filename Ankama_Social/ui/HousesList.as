package Ankama_Social.ui
{
    ...
    import d2actions.HouseTeleportRequest;
    import d2hooks.*;
    import d2actions.*;
    ...
    public class HousesList 
    {
            ...
            switch (target)
            {
                ...
                default:
                    if (target.name.indexOf("lbl_guildShare") != -1)
                    {
                        ...
                        if (data)
                        {
                            this.sysApi.dispatchHook(OpenHouseGuildManager, data);
                        };
                    }
                    else
                    {
                        if (target.name.indexOf("btn_join") != -1)
                        {
                            data = this._interactiveComponentsList[target.name];
                            if (data)
                            {
                                this.sysApi.sendAction(new HouseTeleportRequest(data.houseId, data.houseInstances[0].id));
                            };
                        };
                    };
            };
        ...
        public function onRollOver(target:Object):void
        {
            ...
            {
                if (target.name.indexOf("tx_isOnSale") != -1)
                {
                    data = this._interactiveComponentsList[target.name];
                    if (data)
                    {
                ...
                {
                    if (target.name.indexOf("ctr_emblem") != -1)
                    {
                        data = this._interactiveComponentsList[target.name];
                        if (data)
                        {
                    ...
                    else
                    {
                        if (target.name.indexOf("lbl_guildShare") != -1)
                        {
                            ...
                            {
                                if (((!(this._myGuild)) && (!(data.houseInstances[0].guildIdentity))))
                                {
