package mx.managers
{
    ...
    public class SystemManager extends MovieClip implements IChildList, IFlexDisplayObject, IFlexModuleFactory, ISystemManager 
    {
        ...
        final mx_internal function $addEventListener(_arg_1:String, listener:Function, useCapture:Boolean=false, priority:int=0, useWeakReference:Boolean=false):void
        {
        ...
        final mx_internal function $removeEventListener(_arg_1:String, listener:Function, useCapture:Boolean=false):void
        {
        ...
        override public function removeEventListener(_arg_1:String, listener:Function, useCapture:Boolean=false):void
        {
            ...
            if (((_arg_1 == FlexEvent.RENDER) || (_arg_1 == FlexEvent.ENTER_FRAME)))
            {
                if (_arg_1 == FlexEvent.RENDER)
                {
                    _arg_1 = Event.RENDER;
                }
                else
                {
                    _arg_1 = Event.ENTER_FRAME;
                };
                try
                {
                    if (this.stage)
                    {
                        this.stage.removeEventListener(_arg_1, listener, useCapture);
                    };
                }
                catch(error:SecurityError)
                {
            ...
            if ((((((_arg_1 == MouseEvent.MOUSE_MOVE) || (_arg_1 == MouseEvent.MOUSE_UP)) || (_arg_1 == MouseEvent.MOUSE_DOWN)) || (_arg_1 == Event.ACTIVATE)) || (_arg_1 == Event.DEACTIVATE)))
            {
                if (!hasEventListener(_arg_1))
                {
                    try
                    {
                        if (this.stage)
                        {
                            this.stage.removeEventListener(_arg_1, this.stageEventHandler, false);
                        };
                    }
                    catch(error:SecurityError)
                    {
            ...
            if (_arg_1 == SandboxMouseEvent.MOUSE_UP_SOMEWHERE)
            {
        ...
        private function stageEventHandler(event:Event):void
        {
            ...
            var mouesCatcherLocalPoint:Point;
