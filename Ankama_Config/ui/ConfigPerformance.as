package Ankama_Config.ui
{
    ...
    public class ConfigPerformance extends ConfigUi 
    {
        ...
        public function main(args:*):void
        {
            ...
            sysApi.addHook(GameFightJoin, this.onGameFightJoin);
            sysApi.addHook(GameFightEnd, this.onGameFightEnd);
            ...
            {
                if (cacheValue == MEDIUM_CACHE_SIZE)
                {
                    index = 1;
                }
                else
                {
                    if (cacheValue == HIGH_CACHE_SIZE)
                    {
                        index = 2;
                    }
                    else
        ...
        {
            var currentValueInCache:int;
            var limit:int;
            var animationLimitText:String;
            var animationLimit:uint;
            var propertyHadBeenChanged:Boolean;
            switch (target)
            {
                ...
                case this.cb_animationsInCache:
                    ...
                    {
                        if (animationLimitText == this.MEDIUM_TEXT)
                        {
                            animationLimit = MEDIUM_CACHE_SIZE;
                        }
                        else
                        {
                            if (animationLimitText == this.HIGH_TEXT)
                            {
